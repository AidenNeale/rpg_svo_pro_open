...
TEST(okvisTestSuite, PoseError) {
  ...
  for (size_t i = 0; i < n_poses; ++i) {
    ...
    T.setRandom(svo::test_utils::sampleUniformRealDistribution<double>(deterministic),
    ...
    map.addParameterBlock(pose_parameter_block, svo::ceres_backend::Map::Pose6d);
    ...
    // add it
    ceres::ResidualBlockId id = map.addResidualBlock(pose_error, nullptr, pose_parameter_block);
    // check Jacobian
