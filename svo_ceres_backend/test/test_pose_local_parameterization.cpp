...
TEST(poseLocalParametrizationTests, PlusTest) {
  ...
  svo::Transformation T_dist(Eigen::Vector3d(x_dist[0], x_dist[1], x_dist[2]),
                             Eigen::Quaterniond(x_dist[6], x_dist[3], x_dist[4], x_dist[5]));
  svo::Quaternion q_dist_with_ze = svo::Quaternion::exp(dx.tail<3>()) * T.getRotation();
  EXPECT_TRUE(
      EIGEN_MATRIX_EQUAL_DOUBLE(T_dist.getRotationMatrix(), q_dist_with_ze.getRotationMatrix()));
  EXPECT_TRUE(EIGEN_MATRIX_EQUAL_DOUBLE(T_dist.getPosition(), T.getPosition() + dx.head<3>()));
}
...
TEST(poseLocalParametrizationTests, PlusJacobianTest) {
  ...
  x.tail<4>().normalize();
  Eigen::Matrix<svo::FloatType, 6, 1> dx = Eigen::Matrix<svo::FloatType, 6, 1>::Zero();
  Eigen::Matrix<double, 7, 6, Eigen::RowMajor> jacobian_analytic;
  svo::ceres_backend::PoseLocalParameterization::plusJacobian(x.data(), jacobian_analytic.data());
  ...
  for (size_t i = 0; i < 6; ++i) {
    dx[i] = delta;
    svo::ceres_backend::PoseLocalParameterization::plus(x.data(), dx.data(), result_plus.data());
    dx[i] = -delta;
    svo::ceres_backend::PoseLocalParameterization::plus(x.data(), dx.data(), result_minus.data());
...
TEST(poseLocalParametrizationTests, LiftJacobianTest) {
  ...
  svo::ceres_backend::PoseLocalParameterization::plusJacobian(x.data(), plus_jacobian.data());
