...
TEST(okvisTestSuite, SpeedAndBiasError) {
  ...
  for (size_t i = 0; i < n_terms; ++i) {
    ...
    std::shared_ptr<svo::ceres_backend::SpeedAndBiasParameterBlock> speedandbias_parameter_block =
        std::make_shared<svo::ceres_backend::SpeedAndBiasParameterBlock>(speed_and_bias, i);
    // add it as optimizable thing.
    map.addParameterBlock(speedandbias_parameter_block, svo::ceres_backend::Map::Trivial);
    map.setParameterBlockVariable(i);
    // invent an error
    std::shared_ptr<svo::ceres_backend::SpeedAndBiasError> speedandbias_error =
        std::make_shared<svo::ceres_backend::SpeedAndBiasError>(speed_and_bias, 1.0, 1.0, 1.0);
    // add it
    ceres::ResidualBlockId id =
        map.addResidualBlock(speedandbias_error, nullptr, speedandbias_parameter_block);
    // check Jacobian
