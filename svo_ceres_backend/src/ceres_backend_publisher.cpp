...
void CeresBackendPublisher::publishImuPose(const ViNodeState& state, const int64_t timestamp,
                                           const int32_t seq) {
  // Trace state
  state_ = state;
...
void CeresBackendPublisher::publishBackendLandmarks(const int64_t timestamp) const {
  ...
  for (auto& it : idmap) {
    if (it.second->typeInfo() == "HomogeneousPointParameterBlock" && !it.second->fixed()) {
      n_pts++;
