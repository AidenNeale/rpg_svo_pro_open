...
bool MapAlignmentSE3::getTransformation(Transformation& T_old_new) {
  ...
  for (int i = 0; i < num_points_; ++i) {
    /* Don't close loop if associated points are too far apart. This can happen if SVO didn't assign
     * good points in the beginning for example. ToDo: Find a better way for outlier rejection
     */
  ...
  double residual = 0;
  for (int i = 0; i < num_points_; ++i) {
    double curr_res = (points_new_no_outliers.col(i) - R * points_old_no_outliers.col(i) -
  ...
  //  Eigen::JacobiSVD<Eigen::Matrix<FloatType,3,3> > svd(
  //          W, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::BDCSVD<Eigen::Matrix<FloatType, 3, 3> > svd1(W1,
  ...
  residual = 0;
  for (int i = 0; i < num_points_; ++i) {
    double curr_res = (points_new_no_outliers_newit.col(i) -
...
bool MapAlignmentSE3::getTransformRansac(Transformation& T_old_new, const int& min_num_3d,
  ...
  std::shared_ptr<opengv::sac_problems::point_cloud::PointCloudSacProblem> relposeproblem_ptr(
  ...
  for (size_t i = 0; i < num_inliers; i++) {
    res += (points_vec_new_[ransac.inliers_[i]] - R * points_vec_old_[ransac.inliers_[i]] -
            transform.col(3))
               .norm();
  }
...
bool MapAlignmentSE3::solveJointOptimisation(
    ...
    const size_t& num_bow_features_lc, const size_t& num_bow_features_cf, const int& min_num_3d) {
  ...
  for (const std::pair<size_t, size_t>& correspondence : point_correspondences) {
    ...
    P_w << (double)landmarks_lc[correspondence.first].x,
        (double)landmarks_lc[correspondence.first].y, (double)landmarks_lc[correspondence.first].z;
    Eigen::Matrix<double, 3, 1> P_c;
    P_c = T_w_lc.inverse() * P_w;
    ceres::CostFunction* cost_function =
        ceres_backend::ReprojectionErrorSimple::Create(sqrt_information, p, P_c);
    problem.AddResidualBlock(cost_function, loss_function, t.data(), q.coeffs().data());
  }
...
Transformation MapAlignmentSE3::getTransformationCombined(Transformation& T_w_lc,
                                                          Transformation& T_w_cf) {
  Transformation T_new_old, T_w_new;
  T_w_new = T_w_lc * t_rel_combined_;
  /* Note that this T_new_old transforms the T_cf (current frame or sliding window), to its new
