...
namespace svo {
CeresBackendInterface::CeresBackendInterface(const CeresBackendInterfaceOptions& options,
  ...
  if (options_.use_outlier_rejection) {
    outlier_rejection_.reset(new OutlierRejection(options_.outlier_rejection_px_threshold));
  }
  // Cameras -------------------------------------------------------------------
  ...
  if (options.refine_extrinsics) {
    ExtrinsicsEstimationParametersVec extrinsics_estimation_parameters(
        ...
        ExtrinsicsEstimationParameters(
            options.extrinsics_pos_sigma_meter, options.extrinsics_rot_sigma_rad,
            FLAGS_extrinsics_sigma_rel_translation, FLAGS_extrinsics_sigma_rel_orientation));
    backend_.addCameraBundle(extrinsics_estimation_parameters, camera_bundle);
  } else {
    ExtrinsicsEstimationParametersVec extrinsics_estimation_parameters(
        camera_bundle->getNumCameras(), ExtrinsicsEstimationParameters(0.0, 0.0, 0.0, 0.0));
    backend_.addCameraBundle(extrinsics_estimation_parameters, camera_bundle);
  }
...
void CeresBackendInterface::loadMapFromBundleAdjustment(const FrameBundlePtr& new_frames,
...
void CeresBackendInterface::addLandmarksAndObservationsToBackend(const FramePtr& frame) {
  // Statistics.
  ...
  for (size_t kp_idx = 0; kp_idx < frame->numFeatures(); ++kp_idx) {
    ...
    if (backend_.isPointInEstimator(point->id())) {
      ...
      if (isFixedLandmark(type)) {
        kp_idx_to_n_obs_map_fixed_lm.emplace_back(std::make_pair(kp_idx, point->obs_.size()));
        continue;
      }
  ...
  // for fixed landmarks
  std::sort(kp_idx_to_n_obs_map_fixed_lm.begin(), kp_idx_to_n_obs_map_fixed_lm.end(),
  ...
  for (size_t idx = 0; idx < kp_idx_to_n_obs_map_fixed_lm.size(); idx++) {
    ...
    n_added_fixed_lm++;
    if (backend_.numFixedLandmarks() >= optimizer_options_.max_fixed_lm_in_ceres_) {
      break;
    }
  }
...
void CeresBackendInterface::updateFrameStateWithBackend(const FramePtr& f,
...
void CeresBackendInterface::updateBundleStateWithBackend(const FrameBundlePtr& frames,
                                                         const bool get_speed_bias) {
  Transformation T_WS;
  ...
  while (!stop_thread_) {
    {
      ...
      vk::Timer timer;
      {
        ...
        if (is_w_T_valid_) {
          ...
          backend_.transformMap(w_T_correction_to_apply_,
      ...
      timer.start();
      {
        if (skip_optimization_once_) {
          ...
          backend_.optimize(optimizer_options_.num_iterations, optimizer_options_.num_threads,
      ...
      if (g_permon_backend_) {
        g_permon_backend_->log("tot_time", timers_[last_optimized_nframe_.load()].stop());
      ...
      if (publisher_) {
        publisher_->publish(last_state_, last_added_frame_stamp_ns_, last_optimized_nframe_.load());
      }
...
void CeresBackendInterface::setImu(const std::shared_ptr<ImuHandler> imu_handler) {
...
void CeresBackendInterface::startTimer(const BundleId bundle_id) { timers_[bundle_id].start(); }
...
void CeresBackendInterface::getLatestSpeedBiasPose(Eigen::Matrix<double, 9, 1>* speed_bias,
                                                   Transformation* T_WS, double* timestamp) const {
...
void CeresBackendInterface::setReinitStartValues(const Eigen::Matrix<double, 9, 1>& sb,
