...
class CallbackHost {
 public:
  ...
  void addCallback(const Callback& callback) {
    if (!callback) {
      throw std::invalid_argument("Callback isn't a valid function!");
    }
    if (!callbacks_.emplace(callback).second) {
      throw std::runtime_error("Duplicate callback addition!");
    }
  }
  ...
  void linkBatchWorker(BatchWorker<CallbackArgs...>* worker) {
    ...
    addCallback([worker](CallbackArgs... args) { worker->addTask(args...); });
  }
...
struct hash<std::function<void(CallbackArgs...)>> {
  size_t operator()(const std::function<void(CallbackArgs...)>& x) const {
    ...
    void (*const* pointer)(CallbackArgs...) = x.template target<void (*)(CallbackArgs...)>();
...
bool operator==(const std::function<void(CallbackArgs...)>& a,
  ...
  if (!a) {
    throw std::invalid_argument("Callback a isn't a valid function!");
  }
  if (!b) {
    throw std::invalid_argument("Callback b isn't a valid function!");
  }
  void (*const* a_pointer)(CallbackArgs...) = a.template target<void (*)(CallbackArgs...)>();
  void (*const* b_pointer)(CallbackArgs...) = b.template target<void (*)(CallbackArgs...)>();
