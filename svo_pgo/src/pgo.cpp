...
void Pgo::addPoseToPgoProblem(const Transformation t, const int frame_id) {
  if (!problem_lock_) {
    ...
    std::cerr << "###### Warning: Problem is locked, not adding any poses "
                 "#########";
  }
}

void Pgo::addSequentialConstraintToPgoProblem(const Transformation t_be, const int frame_id_b,
  ...
  ceres::MapOfPoses::iterator pose_begin_iter = poses_->find(constraint.id_begin);
  ...
  if (searchKfIdInQueue(frame_id_e)) {
    std::cout << "Frame Id " << frame_id_e << " added with low weight ";
  ...
  if (!problem_lock_) {
    ...
    std::cout << "Sequential constraint added for frame " << constraint.id_begin << " and "
              << constraint.id_end;
    problem_.SetManifold(pose_begin_iter->second.q.coeffs().data(), quaternion_manifold_.get());
    problem_.SetManifold(pose_end_iter->second.q.coeffs().data(), quaternion_manifold_.get());
  } else {
    std::cerr << "###### Warning: Problem is locked, not adding any sequential "
                 "constraints #########";
  }
}

void Pgo::addLoopConstraintToPgoProblem(const Transformation t_be, const int frame_id_b,
                                        const int frame_id_e) {
  ceres::Constraint3d constraint;
  constraint.id_begin = frame_id_b;
  constraint.id_end = frame_id_e;
  ...
  if (!problem_lock_) {
    ...
    problem_.SetParameterBlockConstant(pose_begin_iter->second.q.coeffs().data());
    ...
    problem_.SetParameterBlockVariable(pose_begin_iter->second.q.coeffs().data());
    has_updated_result_ = true;
    problem_lock_ = true;
    problem_.RemoveResidualBlock(res_block_id);
  } else {
    std::cerr << "###### Warning: Problem is locked, not adding any loop "
                 "constraints #########";
  }
}
...
void Pgo::addConstraint(const Transformation& t_be, const int& frame_id_b, const int& frame_id_e,
  ...
  if (pose_begin_iter == poses_->end()) {
    throw std::runtime_error("Pose with ID: " + std::to_string(constraint.id_begin) +
                             " not found.");
  }
  ceres::MapOfPoses::iterator pose_end_iter = poses_->find(constraint.id_end);

  if (pose_end_iter == poses_->end()) {
    throw std::runtime_error("Pose with ID: " + std::to_string(constraint.id_end) + " not found.");
  }
  const Eigen::Matrix<double, 6, 6> sqrt_information = constraint.information.llt().matrixL();
  // Ceres will take ownership of the pointer.
  ceres::CostFunction* cost_function =
      ceres::PoseGraph3dErrorTerm::Create(constraint.t_be, sqrt_information);
  problem_.AddResidualBlock(cost_function, loss_function_, pose_begin_iter->second.p.data(),
...
void Pgo::solve() {
  ...
  if (pose_start_iter == poses_->end()) {
    throw std::runtime_error("There are no poses.");
  }
