...
BaseOptions loadBaseOptions(std::shared_ptr<rclcpp::Node> pnh, bool forward_default) {
  BaseOptions o;
  o.max_n_kfs = vk::param<int>(pnh, "max_n_kfs", 5);
  o.use_imu = vk::param<bool>(pnh, "use_imu", false);
  o.trace_dir = vk::param<std::string>(
      pnh, "trace_dir", ament_index_cpp::get_package_share_directory("svo") + "/trace");
  ...
  o.backend_scale_stable_thresh = vk::param<double>(pnh, "backend_scale_stable_thresh", 0.02);
  o.global_map_lc_timeout_sec_ = vk::param<double>(pnh, "global_map_timeout_sec", 2.0);
  return o;
}

DetectorOptions loadDetectorOptions(std::shared_ptr<rclcpp::Node> pnh) {
  DetectorOptions o;
...
DepthFilterOptions loadDepthFilterOptions(std::shared_ptr<rclcpp::Node> pnh) {
  ...
  o.max_n_seeds_per_frame =
      static_cast<size_t>(static_cast<double>(vk::param<int>(pnh, "max_fts", 120)) *
                          vk::param<double>(pnh, "max_seeds_ratio", 3.0));
  o.max_map_seeds_per_frame =
      static_cast<size_t>(static_cast<double>(vk::param<int>(pnh, "max_map_fts", 120)));
...
InitializationOptions loadInitializationOptions(std::shared_ptr<rclcpp::Node> pnh) {
...
FeatureTrackerOptions loadTrackerOptions(std::shared_ptr<rclcpp::Node> pnh) {
...
ReprojectorOptions loadReprojectorOptions(std::shared_ptr<rclcpp::Node> pnh) {
...
CameraBundle::Ptr loadCameraFromYaml(std::shared_ptr<rclcpp::Node> pnh) {
  ...
  for (const auto& cam : ncam->getCameraVector()) cam->printParameters(std::cout, "");
  return ncam;
}

#ifdef SVO_LOOP_CLOSING
MapAlignmentOptions loadMapAlignmentOptions(std::shared_ptr<rclcpp::Node> pnh) {
  MapAlignmentOptions o;
  o.ransac3d_inlier_percent = vk::param<double>(pnh, "ransac3d_inlier_percent", 40.0);
  o.ransac3d_min_pts = vk::param<int>(pnh, "ransac3d_min_pts", 8);

  return o;
}

LoopClosureOptions loadLoopClosureOptions(std::shared_ptr<rclcpp::Node> pnh) {
  ...
  std::string default_voc_path =
      ament_index_cpp::get_package_share_directory("svo_online_loopclosing") + "/vocabularies/";
  ...
  o.scale_ret_app = vk::param<std::string>(pnh, "scale_retrieval_approach", "None");
  o.global_map_type = vk::param<std::string>(pnh, "global_map_type", "BuiltInPoseGraph");
...
LoopClosingPtr getLoopClosingModule(std::shared_ptr<rclcpp::Node> pnh, const CameraBundlePtr& cam) {
  // Create camera
  ...
  lc->map_alignment_se3_ = std::make_shared<MapAlignmentSE3>(loadMapAlignmentOptions(pnh));
  return lc;
}
#endif

#ifdef SVO_GLOBAL_MAP
GlobalMapPtr getGlobalMap(std::shared_ptr<rclcpp::Node> pnh, const CameraBundlePtr& ncams) {
  GlobalMapOptions o;
  ...
  o.init_min_lm_obs_frame = static_cast<size_t>(vk::param<int>(pnh, "gm_init_min_lm_obs_frame", 5));
  o.debug_fix_all_poses = vk::param<bool>(pnh, "gm_debug_fix_all_poses", false);
  ...
  o.ignore_redundant_landmarks = vk::param<bool>(pnh, "gm_ignore_redundant_landmarks", false);
  o.ignore_grid_size = vk::param<int>(pnh, "gm_ignore_grid_size", 30);
  o.point_stable_thresh_meter = vk::param<double>(pnh, "gm_point_stable_thresh_meter", 0.05);
  o.inc_version_stable_ratio = vk::param<double>(pnh, "gm_inc_version_stable_ratio", 0.9);
  ...
  o.max_reprojected_fixed_landmark_each_frame =
      static_cast<size_t>(vk::param<int>(pnh, "gm_max_reprojected_fixed_landmark_each_frame", 20));
  o.fix_lm_reobs_threshold = vk::param<int>(pnh, "gm_fix_lm_reobs_threshold", 5);

  o.min_num_obs = static_cast<size_t>(vk::param<int>(pnh, "gm_min_num_obs", 2));
  o.pt_cond_thresh = vk::param<double>(pnh, "gm_pt_cond_thresh", -1);
  o.pt_sigma_thresh = vk::param<double>(pnh, "gm_pt_sigma_thresh", 0.1);
  o.add_range_prior_to_good_pt = vk::param<bool>(pnh, "gm_add_range_prior_to_good_pt", true);
  o.max_point_age = vk::param<int>(pnh, "gm_max_point_age", 20);
  o.min_parallax_thresh_rad = vk::param<double>(pnh, "gm_min_parallax_deg", 5.0) / 180.0 * M_PI;
  o.use_edgelet = vk::param<bool>(pnh, "gm_use_edgelet", false);
  o.use_map_point = vk::param<bool>(pnh, "gm_use_map_point", false);
  o.mappoint_min_obs = static_cast<size_t>(vk::param<int>(pnh, "gm_map_point_min_obs", 4));

  o.use_relative_pose_fixation = vk::param<bool>(pnh, "gm_use_relative_pose_fixation", true);
  o.relative_prior_with_imu_min_time_sec =
      vk::param<double>(pnh, "gm_relative_prior_with_imu_min_time_sec", 1.0);
  o.frame_fixation_release_check = vk::param<bool>(pnh, "gm_frame_fixation_release_check", true);
  o.relative_min_common = static_cast<size_t>(vk::param<int>(pnh, "gm_relative_min_common_obs", 6));
  o.relative_release_min_common =
      static_cast<size_t>(vk::param<int>(pnh, "gm_relative_release_min_common_obs", 30));
  o.relative_pos_sigma_meter = vk::param<double>(pnh, "gm_relative_pos_sigma_meter", 0.1);
  o.relative_rot_sigma_rad =
      vk::param<double>(pnh, "gm_relative_rot_sigma_deg", 5.0) / 180.0 * M_PI;

  o.query_frame_min_3d_points =
      static_cast<size_t>(vk::param<int>(pnh, "gm_query_frame_min_3d_points", 20));

  GraphManagerOptions gm_o;
  gm_o.use_bearing_factor = vk::param<bool>(pnh, "ba_use_bearing_factor", true);
  gm_o.reproj_error_ns_px = vk::param<double>(pnh, "ba_reproj_error_noise_px", 1.0);
  gm_o.smart_reproj_outlier_thresh_px =
      vk::param<double>(pnh, "ba_smart_reproj_outlier_thresh_px", 3.0);
  gm_o.use_robust_px_noise = vk::param<bool>(pnh, "ba_use_robust_px_noise", true);
  // prior factors
  gm_o.position_prior_sigma_meter = vk::param<double>(pnh, "ba_pos_sigma_meter", 0.001);
  gm_o.init_pos_sigma_meter = gm_o.position_prior_sigma_meter;
  gm_o.rotation_prior_sigma_rad = vk::param<double>(pnh, "ba_rot_sigma_degree", 1.0) / 180.0 * M_PI;
  ...
  gm_o.point_prior_sigma_meter = vk::param<double>(pnh, "ba_point_sigma_meter", 0.001);
  ...
  gtsam_opt_options.output_errors = vk::param<bool>(pnh, "ba_output_errors", false);
  gtsam_opt_options.trace_dir = vk::param<std::string>(
      pnh, "trace_dir", ament_index_cpp::get_package_share_directory("svo") + "/trace");
  gtsam_opt_options.optim_method =
      vk::param<std::string>(pnh, "isam_optimization_method", std::string("GaussNewton"));
  gtsam_opt_options.isam_relinearize_thresh =
      vk::param<double>(pnh, "isam_relinearize_thresh", 0.1);
  gtsam_opt_options.isam_relinearize_skip = vk::param<int>(pnh, "isam_relinearize_skip", 10);
  gtsam_opt_options.isam_wildfire_thresh = vk::param<double>(pnh, "isam_wildfire_thresh", 0.001);
  gtsam_opt_options.isam_detailed_results = vk::param<bool>(pnh, "isam_detailed_results", false);

  GlobalMapPtr gm_ptr = std::make_shared<GlobalMap>(o, gm_o, gtsam_opt_options, cams);

  return gm_ptr;
}
#endif

StereoTriangulationOptions loadStereoOptions(std::shared_ptr<rclcpp::Node> pnh) {
...
ImuHandler::Ptr getImuHandler(std::shared_ptr<rclcpp::Node> pnh) {
  ...
  options.temporal_stationary_check = vk::param<bool>(pnh, "imu_temporal_stationary_check", false);
  options.temporal_window_length_sec_ =
      vk::param<double>(pnh, "imu_temporal_window_length_sec", 0.5);
  options.stationary_acc_sigma_thresh_ = vk::param<double>(pnh, "stationary_acc_sigma_thresh", 0.0);
  options.stationary_gyr_sigma_thresh_ = vk::param<double>(pnh, "stationary_gyr_sigma_thresh", 0.0);
  ImuHandler::Ptr imu_handler(new ImuHandler(imu_calib, imu_init, options));
  return imu_handler;
}

void setInitialPose(std::shared_ptr<rclcpp::Node> pnh, FrameHandlerBase& vo) {
  Transformation T_world_imuinit(Quaternion(vk::param<double>(pnh, "T_world_imuinit/qw", 1.0),
...
FrameHandlerMono::Ptr makeMono(std::shared_ptr<rclcpp::Node> pnh, const CameraBundlePtr& cam) {
  // Create camera
  CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
  ...
  FrameHandlerMono::Ptr vo = std::make_shared<FrameHandlerMono>(
      loadBaseOptions(pnh, false), loadDepthFilterOptions(pnh), loadDetectorOptions(pnh),
      loadInitializationOptions(pnh), loadReprojectorOptions(pnh), loadTrackerOptions(pnh), ncam);
...
FrameHandlerStereo::Ptr makeStereo(std::shared_ptr<rclcpp::Node> pnh, const CameraBundlePtr& cam) {
  // Load cameras
  CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
  ...
  FrameHandlerStereo::Ptr vo = std::make_shared<FrameHandlerStereo>(
      loadBaseOptions(pnh, true), loadDepthFilterOptions(pnh), loadDetectorOptions(pnh),
      init_options, loadStereoOptions(pnh), loadReprojectorOptions(pnh), loadTrackerOptions(pnh),
      ncam);
...
FrameHandlerArray::Ptr makeArray(std::shared_ptr<rclcpp::Node> pnh, const CameraBundlePtr& cam) {
  // Load cameras
  CameraBundle::Ptr ncam = (cam) ? cam : loadCameraFromYaml(pnh);
  ...
  FrameHandlerArray::Ptr vo = std::make_shared<FrameHandlerArray>(
      loadBaseOptions(pnh, true), depth_filter_options, loadDetectorOptions(pnh), init_options,
