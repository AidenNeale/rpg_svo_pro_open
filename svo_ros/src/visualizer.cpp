...
Visualizer::Visualizer(const std::string& trace_dir, std::shared_ptr<rclcpp::Node> nh_private,
    ...
    : pnh_(nh_private),
      ...
      publish_points_display_time_(
          rclcpp::Duration::from_seconds(vk::param<double>(pnh_, "publish_point_display_time", 0))),
...
void Visualizer::publishLineList(
    std::shared_ptr<rclcpp::Publisher<visualization_msgs::msg::Marker>> pub,
    const rpg::Aligned<std::vector, Eigen::Matrix<float, 1, 6>>& links, const std::string& ns,
    const Eigen::Vector3f& color, const double scale, const double alpha) {
  visualization_msgs::msg::Marker marker;
  marker.id = 0;
  marker.ns = ns;
  marker.header.frame_id = svo::Visualizer::kWorldFrame;
  marker.header.stamp = pnh_->now();
...
void Visualizer::publishPositionVecAsPC(
  ...
  for (const auto& positions : vec_of_position_vec) total_points += positions.size();
  ...
  for (size_t idx = 0; idx < vec_of_position_vec.size(); ++idx) {
    ...
    for (size_t i = 0; i < positions.size(); ++i, ++iter_x, ++iter_y, ++iter_z, ++iter_intensity) {
...
void Visualizer::publishStringsAtPositions(
...
void Visualizer::publishSvoInfo(const svo::FrameHandlerBase* const svo,
  ...
  if (pub_info_->get_subscription_count() == 0) return;
...
void Visualizer::publishImuPose(const Transformation& T_world_imu,
                                ...
                                const int64_t timestamp_nanoseconds) {
  if (pub_imu_pose_->get_subscription_count() == 0) return;
  ...
  for (size_t i = 0; i < 36; ++i) msg_pose.pose.covariance[i] = Covariance(i % 6, i / 6);
  pub_imu_pose_->publish(msg_pose);
}

void Visualizer::publishCameraPoses(const FrameBundlePtr& frame_bundle,
                                    const int64_t timestamp_nanoseconds) {
  vk::output_helper::publishTfTransform(frame_bundle->at(0)->T_cam_world(),
                                        rclcpp::Time(timestamp_nanoseconds), "cam_pos", kWorldFrame,
                                        br_);

  for (size_t i = 0; i < frame_bundle->size(); ++i) {
    if (pub_cam_poses_.at(i)->get_subscription_count() == 0) return;
...
void Visualizer::publishBundleFeatureTracks(const FrameBundlePtr frames_ref,
                                            const FrameBundlePtr frames_cur, int64_t timestamp) {
  ...
  for (size_t i = 0; i < frames_ref->size(); ++i) {
    ...
    feature_tracking_utils::getFeatureMatches(*frames_ref->at(i), *frames_cur->at(i),
                                              &matches_ref_cur);
    publishFeatureTracks(frames_ref->at(i)->px_vec_, frames_cur->at(i)->px_vec_, matches_ref_cur,
                         frames_cur->at(i)->img_pyr_, img_pub_level_, timestamp, i);
  }
}

void Visualizer::publishFeatureTracks(const Keypoints& px_ref, const Keypoints& px_cur,
                                      const std::vector<std::pair<size_t, size_t>>& matches_ref_cur,
                                      const ImgPyr& img_pyr, const Level& level,
                                      const uint64_t timestamp, const size_t frame_index) {
  ...
  for (size_t i = 0; i < matches_ref_cur.size(); ++i) {
    ...
    cv::line(img_rgb, cv::Point2f(px_cur(0, i_cur) / scale, px_cur(1, i_cur) / scale),
             cv::Point2f(px_ref(0, i_ref) / scale, px_ref(1, i_ref) / scale), cv::Scalar(0, 255, 0),
             2);
  }
...
void Visualizer::publishImagesWithFeatures(const FrameBundlePtr& frame_bundle,
                                           const int64_t timestamp, const bool draw_boundary) {
  if (trace_id_ % img_pub_nth_ != 0) return;
  ...
  for (size_t i = 0; i < frame_bundle->size(); ++i) {
    ...
    if (draw_boundary) {
      cv::rectangle(img_rgb, cv::Point2f(0.0, 0.0), cv::Point2f(img_rgb.cols, img_rgb.rows),
                    cv::Scalar(0, 255, 0), 6);
    }
    writeCaptionStr(img_rgb);
    cv_bridge::CvImage img_msg;
...
void Visualizer::visualizeHexacopter(const Transformation& T_frame_world,
  ...
  if (pub_frames_->get_subscription_count() > 0) {
    vk::output_helper::publishCameraMarker(pub_frames_, "cam_pos", "cams", rclcpp::Time(timestamp),
...
void Visualizer::visualizeQuadrocopter(const Transformation& T_frame_world,
                                       const uint64_t timestamp) {
  vk::output_helper::publishTfTransform(T_frame_world, rclcpp::Time(timestamp), "cam_pos",
                                        kWorldFrame, br_);

  if (pub_frames_->get_subscription_count() > 0) {
    vk::output_helper::publishQuadrocopterMarkers(
        pub_frames_, "cam_pos", "cams", rclcpp::Time(timestamp), 1, 0, 0.8, Vector3d(0., 0., 1.));
  }
}

void Visualizer::visualizeMarkers(const FrameBundlePtr& frame_bundle,
                                  const std::vector<FramePtr>& close_kfs, const Map::Ptr& map) {
  FramePtr frame = frame_bundle->at(0);  // TODO
  uint64_t timestamp = static_cast<uint64_t>(frame_bundle->getMinTimestampNanoseconds());
...
void Visualizer::publishTrajectoryPoint(const Eigen::Vector3d& pos_in_vision,
  ...
  if (pub_points_->get_subscription_count() > 0) {
    ...
    vk::output_helper::publishPointMarker(
        pub_points_, pos_in_vision, "trajectory", rclcpp::Time(timestamp), id, 0,
...
void Visualizer::publishSeedsAsPointcloud(const Frame& frame, bool only_converged_seeds,
                                          bool reset_pc_before_publishing) {
  if (pub_pc_->get_subscription_count() == 0) return;
  ...
  pc_->header.stamp = pnh_->now().nanoseconds();
  pc_->reserve(frame.num_features_);
  for (size_t i = 0; i < frame.num_features_; ++i) {
    if ((only_converged_seeds && isConvergedCornerEdgeletSeed(frame.type_vec_.at(i))) ||
  ...
  sensor_msgs::msg::PointCloud2 msg;
  pcl::toROSMsg(*pc_, msg);
  msg.header.stamp = pnh_->now();
...
void Visualizer::publishSeedsUncertainty(const Map::Ptr& map) {
  ...
  msg_variance.header.stamp = pnh_->now();
...
void Visualizer::publishMapRegion(const std::vector<FramePtr>& frames) {
  ...
  if (pub_pc_->get_subscription_count() > 0) {
    ...
    pcl_conversions::toPCL(pnh_->now(), pc_->header.stamp);
    pc_->clear();
...
void Visualizer::publishKeyframeWithPoints(const FramePtr& frame, const uint64_t timestamp,
                                           const double marker_scale) {
  // publish keyframe
  Transformation T_world_cam(frame->T_f_w_.inverse());
  vk::output_helper::publishFrameMarker(pub_frames_, T_world_cam.getRotationMatrix(),
                                        T_world_cam.getPosition(), "kfs", pnh_->now(),
                                        frame->id_ * 10, 0, marker_scale * 2.0);
  ...
  for (size_t i = 0; i < frame->num_features_; ++i) {
    ...
    if (isEdgelet(frame->type_vec_[i])) {
      vk::output_helper::publishPointMarker(pub_points_, xyz_world, "pts", pnh_->now(), id, 0,
    ...
    if (trace_pointcloud_) {
      if (!ofs_pointcloud_.is_open()) ofs_pointcloud_.open(trace_dir_ + "/pointcloud.txt");
      ofs_pointcloud_ << xyz_world.x() << " " << xyz_world.y() << " " << xyz_world.z() << std::endl;
    }
    ...
    /*
     ...
     "normal", pnh_->now(), point->id_, 0,  0.005,
     Vector3d(0.0, 0., 1.0));
     }
     */
  }
...
void Visualizer::publishActiveKeyframes(const std::vector<FramePtr>& active_kfs) {
  ...
  for (size_t i = 0; i < active_kfs.size() - 1; i++) {
    vk::output_helper::publishLineMarker(
        pub_frames_, active_kfs[i]->pos(), active_kfs[i + 1]->pos(), ns, pnh_->now(), i,
        0,  // add the marker
        0.8 * trajectory_marker_scale_ * vis_scale_, Eigen::Vector3d(.0, .0, 0.5));
  }
}
...
void Visualizer::visualizeCoordinateFrames(const Transformation& T_world_cam) {
  ...
  // camera frame
  vk::output_helper::publishFrameMarker(pub_markers_, T_world_cam.getRotationMatrix(),
                                        T_world_cam.getPosition(), "cam", pnh_->now(), 0, 0, 0.2);

  // origin frame
  vk::output_helper::publishFrameMarker(pub_markers_, Matrix3d::Identity(), Vector3d::Zero(),
                                        kWorldFrame, pnh_->now(), 0, 0, 0.2);
}

#ifdef SVO_LOOP_CLOSING
void Visualizer::publishLoopClosureInfo(const LoopVizInfoVec& loop_viz_info, const std::string& ns,
                                        const Eigen::Vector3f& color, const double scale) {
...
bool Visualizer::publishPoseGraph(const std::vector<KeyFramePtr>& kf_list,
                                  const bool redo_pointcloud, const size_t ignored_past_frames) {
  bool pc_recalculated = false;
  if (pub_pose_graph_->get_subscription_count() > 0) {
    PointCloud pose_graph_pc;
    pcl_conversions::toPCL(pnh_->now(), pose_graph_pc.header.stamp);
    ...
    // Convert to sensor_msgs::msg::PointCloud2 and publish
    sensor_msgs::msg::PointCloud2 msg;
    pcl::toROSMsg(pose_graph_pc, msg);
    msg.header.stamp = pnh_->now();
    msg.header.frame_id = kWorldFrame;
    pub_pose_graph_->publish(msg);
  }

  if (pub_pose_graph_map_->get_subscription_count() > 0 && kf_list.size() > ignored_past_frames) {
    if (redo_pointcloud) {
      // TODO: we do not need to redo it every time
      pose_graph_map_.clear();
      pcl_conversions::toPCL(pnh_->now(), pose_graph_map_.header.stamp);
    ...
    // Convert to sensor_msgs::msg::PointCloud2 and publish
    sensor_msgs::msg::PointCloud2 msg;
    pcl::toROSMsg(pose_graph_map_, msg);
    msg.header.stamp = pnh_->now();
    msg.header.frame_id = kWorldFrame;
    pub_pose_graph_map_->publish(msg);
  }
...
void Visualizer::writeCaptionStr(cv::Mat img_rgb) {
  if (viz_caption_str_) {
    cv::putText(img_rgb, img_caption_, cv::Point(20, 20), cv::FONT_HERSHEY_COMPLEX_SMALL, 1,
                cv::Scalar(0, 0, 250), 1, cv::LINE_AA);
  }
}
...
void Visualizer::visualizeGlobalMap(const GlobalMap& map, const std::string ns,
  ...
  if (pub_global_map_vis_->get_subscription_count() > 0) {
    VisibilityInfoVec vis;
    map.getVizLinks(&vis, 10);
    publishLineList(pub_global_map_vis_, vis, ns, color, scale, 0.5);
  }

  if (pub_global_map_keypoints_vis_->get_subscription_count() > 0) {
    VisibilityInfoVec vis;
    map.getKeypointsVisLinks(&vis);
    publishLineList(pub_global_map_keypoints_vis_, vis, ns, Eigen::Vector3f(1.0, 1.0, 1.0), scale,
                    0.5);
  }

  if (pub_global_map_kfs_opt_->get_subscription_count() > 0) {
    rpg::PositionVec kf_pos;
    rpg::PositionVec kf_pos_with_priors;
    map.getAllKFPositions(&kf_pos, &kf_pos_with_priors);
    publishPositionVecAsPC(pub_global_map_kfs_opt_, {kf_pos, kf_pos_with_priors}, {60, 120});
  }
  if (pub_global_map_query_kfs_->get_subscription_count() > 0) {
    rpg::PositionVec kf_pos;
    rpg::PositionVec last_queried_kf_pos;
    map.getQueryKFPositions(&kf_pos, &last_queried_kf_pos);
    publishPositionVecAsPC(pub_global_map_query_kfs_, {kf_pos, last_queried_kf_pos}, {60, 120});
  }

  if (pub_global_map_matched_points_->get_subscription_count() > 0) {
    VisibilityInfoVec vis;
    map.getMatchedPointsVisLinks(&vis);
    publishLineList(pub_global_map_matched_points_, vis, ns, Eigen::Vector3f(1.0f, 0.0f, 0.0f),
                    scale, 0.5);
  }

  if (pub_global_map_reobserved_points_->get_subscription_count() > 0) {
    VisibilityInfoVec vis;
    map.getReobservedPointsVisLinks(&vis);
    publishLineList(pub_global_map_reobserved_points_, vis, ns, Eigen::Vector3f(0.0f, 1.0f, 0.0f),
                    scale, 0.5);
  }

  if (pub_global_map_reobserved_points_frontend_->get_subscription_count() > 0) {
    VisibilityInfoVec vis;
  ...
  if (pub_global_map_point_ids_->get_subscription_count() > 0) {
    std::vector<int> ids;
    ...
    publishStringsAtPositions(pub_global_map_point_ids_, id_strs, positions);
  }

  if (pub_global_map_pts_opt_->get_subscription_count() > 0) {
    ...
    publishPositionVecAsPC(pub_global_map_pts_opt_, {unstable_points, points, points_with_priors},
                           {30, 100, 200});
  }
...
void Visualizer::visualizeFixedLandmarks(const FramePtr& frame) {
  ...
  for (size_t ftr_idx = 0; ftr_idx < frame->numFeatures(); ftr_idx++) {
    if (isFixedLandmark(frame->type_vec_[ftr_idx])) {
      ...
      vis.back().block<1, 3>(0, 3) = frame->landmark_vec_[ftr_idx]->pos().transpose().cast<float>();
    }
  }
