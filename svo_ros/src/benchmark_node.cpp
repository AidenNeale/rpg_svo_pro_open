...
class BenchmarkNode : public SvoInterface {
 ...
 public:
  BenchmarkNode(const PipelineType& type, std::shared_ptr<rclcpp::Node> nh);
  ...
  void traceIdTimePose(const Transformation& pose, const size_t id, const double time_sec);

  void traceStatus(const size_t id, const double time_sec);

  void traceSpeedBias(const svo::ViNodeState& state, const size_t id, const double time_sec);

  bool loadNextImages(const std::string& dataset_basedir, std::ifstream& img_fs, double* stamp_sec,
                      size_t* img_id, std::vector<cv::Mat>* imgs);
...
BenchmarkNode::BenchmarkNode(const PipelineType& type, std::shared_ptr<rclcpp::Node> nh)
    : SvoInterface(type, nh), nh_(nh) {
  ...
  safeCreateTrace(kStampedTrajEst + save_suf + "." + kExt, &trace_marked_est_pose_);
  ...
  img_noise_sigma_ = vk::param<double>(nh_, "dataset_noise_sigma", 0.0);
  sleep_us_ = static_cast<size_t>(vk::param<int>(nh_, "sleep_us", 0));

  first_frame_id_ = static_cast<size_t>(vk::getParam<int>(nh_, "svo/dataset_first_frame", 0));
  last_frame_id_ = static_cast<size_t>(vk::getParam<int>(nh_, "svo/dataset_last_frame", 0));

  trace_only_kf_ = vk::getParam<bool>(nh_, "svo/trace_only_keyframes", false);

  blackout_start_id_ = static_cast<size_t>(vk::getParam<int>(nh_, "svo/blackout_first_id", 0));
  blackout_end_id_ = static_cast<size_t>(vk::getParam<int>(nh_, "svo/blackout_last_id", 0));
}

void BenchmarkNode::safeCreateTrace(const std::string& trace_fn, std::ofstream* fs) {
  CHECK_NOTNULL(fs);
...
template <typename MarkerType>
void BenchmarkNode::tracePose(const Transformation& pose, const MarkerType marker) {
  const Eigen::Quaterniond& q = pose.getRotation().toImplementation();
  const Vector3d& p = pose.getPosition();
  trace_marked_est_pose_.precision(20);
  trace_marked_est_pose_ << marker << " ";
  trace_marked_est_pose_ << p.x() << " " << p.y() << " " << p.z() << " " << q.x() << " " << q.y()
                         << " " << q.z() << " " << q.w() << std::endl;
}
...
void BenchmarkNode::traceSpeedBias(const svo::ViNodeState& state, const size_t id,
                                   const double time_sec) {
  trace_speed_bias_ << id << " " << time_sec << " ";
  trace_speed_bias_ << state.get_W_v_B().x() << " " << state.get_W_v_B().y() << " "
                    << state.get_W_v_B().z() << " " << state.getGyroBias().x() << " "
                    << state.getGyroBias().y() << " " << state.getGyroBias().z() << " "
                    << state.getAccBias().x() << " " << state.getAccBias().y() << " "
                    << state.getAccBias().z() << " " << std::endl;
}

bool BenchmarkNode::loadNextImages(const std::string& dataset_basedir, std::ifstream& img_fs,
  ...
  if ((*img_id < first_frame_id_) || (last_frame_id_ > 0 && *img_id >= last_frame_id_) ||
      img_fs.eof()) {
    img_fs.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    return false;
  }
...
void BenchmarkNode::runBenchmark(const std::string& dataset_dir) {
  ...
  if (svo_->global_map_) {
    ...
    std::string trace_ba_name = svo_->options_.trace_dir + "/" + kSaveBA + suffix + "." + kExt;
    std::string trace_extrins_name = svo_->options_.trace_dir + "/gm_extrins" + suffix + "." + kExt;
  ...
  if (svo_->lc_) {
    ...
    trace_pg_name = svo_->options_.trace_dir + "/" + kSavePg + suffix + "." + kExt;
    std::string lc_timing_path, pg_timing_path, numquery_path;
    lc_timing_path = svo_->options_.trace_dir + "/" + "lc_timing" + suffix + "." + kExt;
    pg_timing_path = svo_->options_.trace_dir + "/" + "pg_timing" + suffix + "." + kExt;
    numquery_path = svo_->options_.trace_dir + "/" + "num_queries" + suffix + "." + kExt;
    if (svo_->lc_->pgo_) {
...
void BenchmarkNode::traceGroundtruth(const Transformation& T_w_gt, const double timestamp) {
  ...
  trace_gt_pose_ << p.x() << " " << p.y() << " " << p.z() << " " << q.x() << " " << q.y() << " "
                 << q.z() << " " << q.w() << std::endl;
}
...
void BenchmarkNode::tracePoseKitti(const Transformation& T_w_f) {
  ...
  trace_est_pose_ << T(0, 0) << " " << T(0, 1) << " " << T(0, 2) << " " << T(0, 3) << " " << T(1, 0)
                  << " " << T(1, 1) << " " << T(1, 2) << " " << T(1, 3) << " " << T(2, 0) << " "
                  << T(2, 1) << " " << T(2, 2) << " " << T(2, 3) << " " << std::endl;
}
...
void BenchmarkNode::traceDepthError(const FramePtr& frame, const cv::Mat& depthmap) {
  ...
  for (size_t i = 0; i < frame->numFeatures(); ++i) {
    if (frame->landmark_vec_[i]) {
      ...
      const double depth_true =
          depthmap.at<float>((int)frame->px_vec_(1, i), (int)frame->px_vec_(0, i));
      trace_depth_error_ << frame->id() << " " << depth_estimated - depth_true << std::endl;
    }
  }
}
...
void BenchmarkNode::runBlenderBenchmark(const std::string& dataset_dir, bool depthmap_has_zbuffer) {
  ...
  if (dataset.getNextFrame(n_pyr_levels, ref_frame, &depthmap)) {
    ...
    for (size_t i = 0; i < ref_frame->num_features_; ++i) {
      ...
      if (depthmap_has_zbuffer) {
        ...
        SVO_DEBUG_STREAM("Depth (" << ref_frame->px_vec_(0, i) << " , " << ref_frame->px_vec_(1, i)
                                   << ") = " << depth);
  ...
  while (dataset.getNextFrame(n_pyr_levels, cur_frame, nullptr) && rclcpp::ok()) {
    ...
    publishResults({cur_frame->img()}, nh_->now().nanoseconds());
    ...
    if (frame_count_ == 50) svo_->reprojectors_.at(0)->options_.remove_unconstrained_points = true;
    ...
    if (svo_->getLastFrames() && svo_->isBackendScaleInitialised()) {
      ...
      traceIdTimePose(svo_->getLastFrames()->at(0)->T_f_w_.inverse(),
                      static_cast<size_t>(cur_frame->id()), cur_frame->getTimestampSec());
...
void BenchmarkNode::runKittiBenchmark(const std::string& dataset_dir) {
  ...
  while (dataset_fs.good() && !dataset_fs.eof() && rclcpp::ok()) {
    ...
    publishResults({img_l_8uC1, img_r_8uC1}, nh_->now().nanoseconds());
    if (svo_->getLastFrames()) tracePoseKitti(svo_->getLastFrames()->at(0)->T_world_cam());
  }
...
int main(int argc, char** argv) {
  ...
  auto nh = std::make_shared<rclcpp::Node>("benchmark_node");
  std::string benchmark_dir(vk::param<std::string>(nh, "dataset_directory", "/tmp"));
  if (vk::param<bool>(nh, "dataset_is_kitti", false)) {
    svo::BenchmarkNode benchmark(svo::PipelineType::kStereo, nh);
    benchmark.runKittiBenchmark(benchmark_dir);
  } else if (vk::param<bool>(nh, "dataset_is_blender", false)) {
    // does depthmap contains z values or distance to the camera (default)
    const bool depthmap_has_zbuffer = vk::param<bool>(nh, "depthmap_has_zbuffer", false);
    svo::BenchmarkNode benchmark(svo::PipelineType::kMono, nh);
    benchmark.runBlenderBenchmark(benchmark_dir, depthmap_has_zbuffer);
  } else {
    if (vk::param<bool>(nh, "dataset_is_stereo", true)) {
      svo::BenchmarkNode benchmark(svo::PipelineType::kStereo, nh);
