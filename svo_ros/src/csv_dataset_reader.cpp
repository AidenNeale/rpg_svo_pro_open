...
void checkHeaderAndOpenStream(const std::string& filename, const std::string& header,
                              std::ifstream* fs) {
  CHECK_NOTNULL(fs);
...
void readLine(const std::string& line, const size_t num_elements,
...
CsvDatasetReader::CsvDatasetReader(const std::string& csv_directory,
  ...
  for (size_t i : camera_indices) {
    ...
    loadCameraData(dir, i, 100 * common::conversions::kMilliSecondsToNanoSeconds);
  }

  for (size_t i : track_indices) {
    std::string dir = csv_directory + "/tracks" + std::to_string(i);
    loadFeatureTracksData(dir, i, 80 * common::conversions::kMilliSecondsToNanoSeconds);
  }
...
void CsvDatasetReader::loadImuData(const std::string data_dir, const int64_t playback_delay) {
  const std::string kHeader =
      "#timestamp [ns],w_RS_S_x [rad s^-1],w_RS_S_y [rad s^-1],w_RS_S_z [rad s^-1],a_RS_S_x [m "
      "s^-2],a_RS_S_y [m s^-2],a_RS_S_z [m s^-2]";
  std::ifstream fs;
  ...
  while (std::getline(fs, line)) {
    ...
    buffer_.insert(
        std::make_pair(imu_measurement->timestamp_nanoseconds + playback_delay, imu_measurement));
    ++i;
  }
...
void CsvDatasetReader::loadCameraData(const std::string& data_dir, const size_t camera_index,
  ...
  while (std::getline(fs, line)) {
    ...
    dataset::CameraMeasurement::Ptr camera_measurement(new dataset::CameraMeasurement(
        std::stoll(elements[0]), camera_index, data_dir + "/data/" + elements[1]));

    buffer_.insert(std::make_pair(camera_measurement->timestamp_nanoseconds + playback_delay,
                                  camera_measurement));
    ++i;
  }
