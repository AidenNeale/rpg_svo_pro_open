...
void BackendVisualizer::visualizeFrames(const gtsam::Values& values) {
  ...
  int i = 0;
  for (auto it = frames.begin(); it != frames.end(); ++it) {
    vk::output_helper::publishFrameMarker(pub_markers_, it->value.rotation().matrix(),
                                          it->value.translation().vector(), "keyframes",
...
void BackendVisualizer::visualizePoseCovariance(const Transformation& T_W_B,
                                                const Eigen::Matrix<double, 6, 6>& covariance) {
  if (pub_markers_->get_subscription_count() == 0) return;

  vk::output_helper::publishGtsamPoseCovariance(pub_markers_, T_W_B.getPosition(),
                                                T_W_B.getRotationMatrix(), covariance, "covariance",
                                                trace_id_, 0, 2.0, Eigen::Vector4d(0, 0, 0.5, 0.5));
}
...
void BackendVisualizer::tracePose(std::map<int, int64_t, std::less<int>> frameid_timestamp_map,
  ...
  for (std::pair<int, int64_t> keyval : frameid_timestamp_map) {
    ...
    ofs_states_ << keyval.second << " " << p.x() << " " << p.y() << " " << p.z() << " " << q.x()
...
void BackendVisualizer::traceStates(const gtsam::Values& values) {
  ...
  for (auto b : biases) {
    ...
    const gtsam::Vector& velocity = values.at<gtsam::Vector3>(gtsam::Symbol('v', id));
    const gtsam::Pose3& T_wi = values.at<gtsam::Pose3>(gtsam::Symbol('x', id));
    const Eigen::Quaterniond q(T_wi.rotation().matrix());
    ofs_states_ << trace_id_ << " " << id << " " << bias.accelerometer().x() << " "
                << bias.accelerometer().y() << " " << bias.accelerometer().z() << " "
                << bias.gyroscope().x() << " " << bias.gyroscope().y() << " "
                << bias.gyroscope().z() << " " << velocity.x() << " " << velocity.y() << " "
                << velocity.z() << " " << T_wi.translation().x() << " " << T_wi.translation().y()
                << " " << T_wi.translation().z() << " " << q.x() << " " << q.y() << " " << q.z()
                << " " << q.w() << " " << std::endl;
  }
  ++trace_id_;
}
...
void BackendVisualizer::traceCovariance(const Eigen::Matrix<double, 6, 6>& C,
  ...
  ofs_covariance_ << timestamp << ", " << C(0, 0) << "," << C(0, 1) << "," << C(0, 2) << ","
                  << C(0, 3) << "," << C(0, 4) << "," << C(0, 5) << "," << C(1, 0) << "," << C(1, 1)
                  << "," << C(1, 2) << "," << C(1, 3) << "," << C(1, 4) << "," << C(1, 5) << ","
                  << C(2, 0) << "," << C(2, 1) << "," << C(2, 2) << "," << C(2, 3) << "," << C(2, 4)
                  << "," << C(2, 5) << "," << C(3, 0) << "," << C(3, 1) << "," << C(3, 2) << ","
                  << C(3, 3) << "," << C(3, 4) << "," << C(3, 5) << "," << C(4, 0) << "," << C(4, 1)
