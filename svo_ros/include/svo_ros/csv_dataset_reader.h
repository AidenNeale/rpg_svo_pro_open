...
struct MeasurementBase {
  ...
  MeasurementBase(int64_t timestamp_nanoseconds, MeasurementType type)
      : timestamp_nanoseconds(timestamp_nanoseconds), measurement_type(type) {}
...
struct CameraMeasurement : public MeasurementBase {
  ...
  CameraMeasurement(int64_t timestamp_nanoseconds, size_t camera_index,
  ...
  cv::Mat getImage() const {
    ...
    if (!image.data) {
      throw std::runtime_error("Failed to load image: " + image_path_filename);
    }
    return image;
  }
...
struct FeatureTrackMeasurement : public MeasurementBase {
  ...
  FeatureTrackMeasurement(int64_t timestamp_nanoseconds, size_t camera_index, int track_id,
                          const Eigen::Vector2d& keypoint_measurement, double keypoint_std_dev)
...
class CsvDatasetReader {
 public:
  ...
  CsvDatasetReader(const std::string& csv_directory, const std::vector<size_t> imu_indices,
 ...
 private:
  ...
  void loadCameraData(const std::string& data_dir, const size_t camera_index,
                      int64_t playback_delay);

  void loadFeatureTracksData(const std::string& data_dir, const size_t camera_index,
                             int64_t playback_delay);
