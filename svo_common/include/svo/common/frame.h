...
class Frame {
 public:
  ...
  std::vector<std::pair<int, Position>, Eigen::aligned_allocator<std::pair<int, Position> > >
      key_pts_;  //!< Five features and associated 3D points which are used to detect if two frames
                 //!< have overlapping field of view. store index + depth
  ...
  size_t num_features_ =
      0u;             ///< the vectors below are initialized to the maximum number of feature-tracks
  ...
  SeedStates
      invmu_sigma2_a_b_vec_;  ///< Vector containing all necessary information for seed update.
  ...
  /// Default Constructor
  Frame(const CameraPtr& cam, const cv::Mat& img, const int64_t timestamp_ns,
        const size_t n_pyr_levels);

  /// Constructor without image. Just for testing!
  Frame(const int id, const int64_t timestamp_ns, const CameraPtr& cam,
  ...
  inline bool isValidLandmark(size_t i) const { return (landmark_vec_.at(i) != nullptr); }

  /// Number of successfully tracked features: seeds and actual landmarks.
  inline size_t numTrackedFeatures() const {
    size_t count = 0;
    for (size_t i = 0; i < num_features_; ++i) {
      if ((isValidLandmark(i) && !isFixedLandmark(type_vec_[i]) && !isMapPoint(type_vec_[i])) ||
  ...
  inline size_t numLandmarks() const {
    return static_cast<size_t>(std::count_if(landmark_vec_.begin(), landmark_vec_.end(),
  ...
  inline size_t numTrackedLandmarksInBA() const {
    size_t count = 0;
    for (size_t i = 0; i < num_features_; ++i) {
      if (isValidLandmark(i) && !isFixedLandmark(type_vec_[i]) && landmark_vec_[i]->in_ba_graph_) {
        count++;
      }
    }
    return count;
  ...
  void setKeyPoints();
  inline void resetKeyPoints() { key_pts_.resize(5, std::make_pair(-1, BearingVector())); }
  ...
  bool isVisible(const Eigen::Vector3d& xyz_w, Eigen::Vector2d* px = nullptr) const;
  ...
  inline int getNFrameIndex() const {
    if (nframe_index_ < 0) {
      throw std::runtime_error("NFrame Index not set in frame.");
    }
  ...
  /// set IMU state
  inline void setIMUState(const Eigen::Vector3d& imu_vel_w, const Eigen::Vector3d& gyr_bias,
  ...
  inline static void jacobian_xyz2uv_imu(const Transformation& T_cam_imu,
    ...
    J_proj << 1, 0, -p_in_cam[0] / p_in_cam[2], 0, 1, -p_in_cam[1] / p_in_cam[2];
  ...
  inline static void jacobian_xyz2img_imu(const Transformation& T_cam_imu,
                                          const Eigen::Vector3d& p_in_imu,
  ...
  static void jacobian_xyz2image_imu(const Camera& cam, const Transformation& T_cam_imu,
                                     const Eigen::Vector3d& p_in_imu,
                                     Eigen::Matrix<double, 2, 6>& J);
...
class FrameBundle {
 public:
  ...
  inline int64_t getMinTimestampNanoseconds() const {
    if (frames_.empty()) {
      throw std::runtime_error("FrameBundle::getMinTimestampNanoseconds: No frames in bundle.");
    }
  ...
  inline double getMinTimestampSeconds() const {
    const double cur_t_sec = common::conversions::kNanoSecondsToSeconds *
  ...
  Transformation get_T_W_B() const {
    if (frames_.empty()) {
      throw std::runtime_error("FrameBundle::get_T_W_B: No frames in bundle.");
    }
  ...
  void set_T_W_B(const Transformation& T_W_B) {
    for (const FramePtr& frame : frames_) frame->T_f_w_ = (T_W_B * frame->T_body_cam_).inverse();
  }
...
bool getSceneDepth(const FramePtr& frame, double& depth_median, double& depth_min,
                   double& depth_max);

void computeNormalizedBearingVectors(const Keypoints& px_vec, const Camera& cam, Bearings* f_vec);
