...
inline FloatType mu(const Eigen::Ref<const SeedState>& mu_sigma2_a_b) { return mu_sigma2_a_b(0); }

inline FloatType sigma2(const Eigen::Ref<const SeedState>& mu_sigma2_a_b) {
  return mu_sigma2_a_b(1);
}

inline FloatType a(const Eigen::Ref<const SeedState>& mu_sigma2_a_b) { return mu_sigma2_a_b(2); }

inline FloatType b(const Eigen::Ref<const SeedState>& mu_sigma2_a_b) { return mu_sigma2_a_b(3); }
...
{
  if (depth_max < 0) {
    throw std::invalid_argument("Depth max must be positive");
  }
  if (depth_min < 0) {
    throw std::invalid_argument("Depth min must be positive");
  }
...
inline FloatType getInitSigma2FromMuRange(FloatType mu_range) { return mu_range * mu_range / 36.0; }

inline bool isConverged(const Eigen::Ref<const SeedState>& mu_sigma2_a_b, FloatType mu_range,
                        FloatType sigma2_convergence_threshold) {
  // If initial uncertainty was reduced by factor sigma2_convergence_threshold
  // we accept the seed as converged.
  const FloatType thresh = mu_range / sigma2_convergence_threshold;
...
inline FloatType getSigma2FromDepthSigma(FloatType depth, FloatType depth_sigma) {
  const FloatType sigma =
      0.5 * (1.0 / std::max(0.000000000001, depth - depth_sigma) - 1.0 / (depth + depth_sigma));
  return sigma * sigma;
}
#endif
