...
struct KeypointIdentifier {
  ...
  inline bool operator==(const KeypointIdentifier& other) const {
    if (!(frame.lock() && other.frame.lock())) {
      throw std::runtime_error("KeypointIdentifier: frame is expired");
    }
    return frame.lock().get() == other.frame.lock().get() && frame_id == other.frame_id &&
...
class Point {
 public:
  ...
  enum PointType { TYPE_EDGELET_SEED, TYPE_CORNER_SEED, TYPE_EDGELET, TYPE_CORNER };
  ...
  std::array<int, 8> last_projected_kf_id_;  //!< Flag for the reprojection: don't reproject a pt
                                             //!< twice in the same camera
  PointType type_ = TYPE_CORNER;             //!< Quality of the point.
  int n_failed_reproj_ =
      0;  //!< Number of failed reprojections. Used to assess the quality of the point.
  int n_succeeded_reproj_ =
      0;  //!< Number of succeeded reprojections. Used to assess the quality of the point.
  ...
  bool getCloseViewObs(const Eigen::Vector3d& pos, FramePtr& ref_frame,
  ...
  void updateHessianGradientUnitPlane(const Eigen::Ref<BearingVector>& f,
                                      const Eigen::Vector3d& p_in_f, const Eigen::Matrix3d& R_f_w,
                                      Eigen::Matrix3d& A, Eigen::Vector3d& b, double& new_chi2);
  ...
  inline static void jacobian_xyz2uv(const Eigen::Vector3d& p_in_f, const Eigen::Matrix3d& R_f_w,
  ...
  inline static void jacobian_xyz2f(const Eigen::Vector3d& p_in_f, const Eigen::Matrix3d& R_f_w,
