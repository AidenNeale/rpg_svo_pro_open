...
Point::Point(const Eigen::Vector3d& pos) : Point(PointIdProvider::getNewPointId(), pos) { ; }
...
void Point::addObservation(const FramePtr& frame, const size_t feature_index) {
  ...
  if (it == obs_.end()) {
    ...
    if (it->keypoint_index_ != feature_index) {
      throw std::runtime_error(
          "Point::addObservation: feature index does not match existing observation");
    }
  }
}
...
void Point::initNormal() {
  if (obs_.empty()) {
    throw std::runtime_error("Point::initNormal: Cannot initialise normal without observations");
  }
...
bool Point::getCloseViewObs(const Eigen::Vector3d& framepos, FramePtr& ref_frame,
  ...
  for (const KeypointIdentifier& obs : obs_) {
    if (FramePtr frame = obs.frame.lock()) {
      ...
      SVO_DEBUG_STREAM("could not unlock weak_ptr<Frame> in Point::getCloseViewObs"
                       ...
                       << ", Feature-ID = " << obs.keypoint_index_ << ", Point-Type = " << type_);
      return false;
    }
  }
...
double Point::getTriangulationParallax() const {
  if (obs_.empty()) {
    throw std::runtime_error("Point::getTriangulationParallax: obs_ is empty");
  }
...
bool Point::triangulateLinear() {
  ...
  const Eigen::MatrixXd BiD = f_world * f_world.colwise().squaredNorm().asDiagonal().inverse();
  const Eigen::Matrix3d AxtAx = n_obs * Eigen::Matrix3d::Identity() - BiD * f_world.transpose();
  const Eigen::Vector3d Axtbx =
      p_world.rowwise().sum() - BiD * f_world.cwiseProduct(p_world).colwise().sum().transpose();
...
void Point::updateHessianGradientUnitPlane(const Eigen::Ref<BearingVector>& f,
                                           ...
                                           const Eigen::Matrix3d& R_f_w, Eigen::Matrix3d& A,
                                           Eigen::Vector3d& b, double& new_chi2) {
  svo::Matrix23d J;
      ...
      if (const FramePtr& frame = obs.frame.lock()) {
        if (using_bearing_vector) {
          updateHessianGradientUnitSphere(
              ...
              frame->T_f_w_.getRotation().getRotationMatrix(), A, b, new_chi2);
        } else {
