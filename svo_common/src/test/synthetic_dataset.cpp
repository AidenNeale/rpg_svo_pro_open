...
SyntheticDataset::SyntheticDataset(const std::string& dataset_dir, size_t cam_index,
    ...
    : dataset_dir_(dataset_dir), cam_index_(cam_index), first_frame_id_(first_frame_id) {
  init();
}
  ...
  if (gt_fs_.is_open()) {
    ...
    if (gt_fs_ >> gt_id >> tx >> ty >> tz >> qx >> qy >> qz >> qw) {
      ...
      if (std::abs(q.norm() - 1.0) > 0.000001)
        std::cout << "test_utils::SyntheticDataset: Quaternion norm = " << q.norm();
      q.normalize();
...
void SyntheticDataset::init() {
  ...
  if (!gt_fs_.is_open()) {
    std::cout << "No groundtruth trajectory found.";
  }
  ...
  if (!depth_fs_.is_open()) {
    std::cout << "No depthmaps trajectory found.";
  }
