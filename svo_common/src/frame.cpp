...
Frame::Frame(const CameraPtr& cam, const cv::Mat& img, const int64_t timestamp_ns,
             size_t n_pyr_levels)
...
Frame::Frame(const int id, const int64_t timestamp_ns, const CameraPtr& cam,
...
void Frame::initFrame(const cv::Mat& img, size_t n_pyr_levels) {
  if (!(key_pts_[0].first == -1 && key_pts_[4].first == -1)) {
    throw std::runtime_error("Key points already initialized. Cannot initialize frame again.");
  }
  // check image
  if (img.empty()) {
    throw std::runtime_error("Image is empty. Cannot initialize frame.");
  }
  ...
  if (img.type() == CV_8UC1) {
    ...
    throw std::runtime_error("Unknown image type! Cannot intialize frame.");
  }
...
FeatureWrapper Frame::getFeatureWrapper(size_t index) {
  if (index >= static_cast<size_t>(px_vec_.cols())) {
    throw std::out_of_range("Index out of range in getFeatureWrapper.");
  }
  return FeatureWrapper(type_vec_[index], px_vec_.col(index), f_vec_.col(index),
                        grad_vec_.col(index), score_vec_(index), level_vec_(index),
                        landmark_vec_[index], seed_ref_vec_[index], track_id_vec_(index));
}

FeatureWrapper Frame::getEmptyFeatureWrapper() { return getFeatureWrapper(num_features_); }
...
void Frame::setKeyPoints() {
  ...
  for (size_t i = 0; i < num_features_; ++i) {
    if (landmark_vec_[i] == nullptr || type_vec_[i] == FeatureType::kOutlier) continue;
...
double Frame::getAngleError(double img_err) const { return cam_->getAngleError(img_err); }

void Frame::jacobian_xyz2image_imu(const svo::Camera& cam, const Transformation& T_cam_imu,
...
FrameBundle::FrameBundle(const std::vector<FramePtr>& frames) : frames_(frames) {
...
void createImgPyramid(const cv::Mat& img_level_0, int n_levels, ImgPyr& pyr) {
  if (img_level_0.type() != CV_8U) {
    throw std::runtime_error("Image type must be CV_8U.");
  }
  if (img_level_0.rows <= 0 || img_level_0.cols <= 0) {
    throw std::runtime_error("Image size must be greater than zero.");
  }
  if (n_levels <= 0) {
    throw std::runtime_error("Number of levels must be greater than zero.");
  }
...
void computeNormalizedBearingVectors(const Keypoints& px_vec, const Camera& cam, Bearings* f_vec) {
  if (!f_vec) {
    throw std::runtime_error("f_vec is null.");
  }
  std::vector<bool> success;
  cam.backProject3(px_vec, f_vec, &success);
  for (const bool s : success) {
    if (!s) {
      throw std::runtime_error("Backprojection failed.");
    }
  }
