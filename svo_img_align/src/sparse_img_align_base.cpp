...
SparseImgAlignBase::SparseImgAlignBase(SolverOptions optimization_options,
    ...
    : vk::solver::MiniLeastSquaresSolver<8, SparseImgAlignState, SparseImgAlignBase>(
...
void SparseImgAlignBase::setWeightedPrior(const Transformation& T_cur_ref_prior,
                                          const double alpha_prior, const double beta_prior,
                                          const double lambda_rot, const double lambda_trans,
                                          const double lambda_alpha, const double lambda_beta) {
...
void SparseImgAlignBase::update(const SparseImgAlignState& state_old, const UpdateVector& dx,
...
void SparseImgAlignBase::applyPrior(const SparseImgAlignState& state) {
  if (iter_ == 0) {
    ...
    for (size_t j = 3; j < 6; ++j) H_max_diag_rot = std::max(H_max_diag_rot, std::fabs(H_(j, j)));
  ...
  H_.noalias() += I_prior_;
  g_.head<6>() += I_prior_.block<6, 6>(0, 0) *
