...
SparseImgAlign::SparseImgAlign(SolverOptions optimization_options, SparseImgAlignOptions options)
    : SparseImgAlignBase(optimization_options, options) {
  setPatchSize<SparseImgAlign>(4);
}

size_t SparseImgAlign::run(const FrameBundle::Ptr& ref_frames, const FrameBundle::Ptr& cur_frames) {
  if (ref_frames->empty()) {
    throw std::runtime_error("SparseImgAlign: No reference frames provided.");
  }
  if (ref_frames->size() != cur_frames->size()) {
    throw std::runtime_error(
        "SparseImgAlign: Number of reference frames and current frames do not match.");
  }
  ...
  for (auto frame : ref_frames->frames_) {
    ...
    sparse_img_align_utils::extractFeaturesSubset(*frame, options_.max_level,
                                                  patch_size_with_border_, fts);
  ...
  Transformation T_icur_iref = cur_frames_->at(0)->T_imu_world() * T_iref_world_.inverse();
  ...
  for (size_t i = 0; i < ref_frames_->size(); ++i) {
    sparse_img_align_utils::precomputeBaseCaches(*ref_frames_->at(i), fts_vec_.at(i),
                                                 options_.use_distortion_jacobian, feature_counter,
                                                 uv_cache_, xyz_ref_cache_, jacobian_proj_cache_);
  }

  for (level_ = options_.max_level; level_ >= options_.min_level; --level_) {
    mu_ = 0.1;
    have_cache_ = false;  // at every level, recompute the jacobians
    if (solver_options_.verbose) printf("\nPYRAMID LEVEL %i\n---------------\n", level_);
    optimize(state);
  }
...
double SparseImgAlign::evaluateError(const SparseImgAlignState& state, HessianMatrix* H,
                                     GradientVector* g) {
  if (!have_cache_)  // is reset at every new level.
  {
    ...
    for (size_t i = 0; i < ref_frames_->size(); ++i) {
      sparse_img_align_utils::precomputeJacobiansAndRefPatches(
          ref_frames_->at(i), uv_cache_, jacobian_proj_cache_, level_, patch_size_,
          fts_vec_.at(i).size(), options_.estimate_illumination_gain,
          options_.estimate_illumination_offset, feature_counter, jacobian_cache_,
          ref_patch_cache_);
    }
    have_cache_ = true;
  }
  ...
  for (size_t i = 0; i < ref_frames_->size(); ++i) {
    ...
    sparse_img_align_utils::computeResidualsOfFrame(
        cur_frames_->at(i), level_, patch_size_, fts_vec_.at(i).size(), T_cur_ref, state.alpha,
        state.beta, ref_patch_cache_, xyz_ref_cache_, feature_counter, match_pxs, residual_cache_,
        visibility_mask_);
  }
...
void SparseImgAlign::finishIteration() {
  if (false) {
    ...
    cv::Mat residual_image(cur_frames_->at(cam_index)->img_pyr_.at(level_).size(), CV_32FC1,
                           cv::Scalar(0.0));
    ...
    std::vector<cv::Mat> residual_image_rgb_vec(
        {cv::Mat(residual_image.size(), CV_32FC1, cv::Scalar(1.0)), residual_image,
...
void extractFeaturesSubset(
    const Frame& ref_frame, const int max_level,
  ...
  for (size_t i = 0; i < ref_frame.num_features_; ++i) {
    if (ref_frame.landmark_vec_[i] == nullptr && ref_frame.seed_ref_vec_[i].keyframe == nullptr)
      continue;
    ...
    if (!(u_tl_i < 0 || v_tl_i < 0 || u_tl_i + patch_size_wb >= cols_minus_two ||
...
void precomputeBaseCaches(const Frame& ref_frame, const std::vector<size_t>& fts,
                          const bool use_distortion_jac, size_t& feature_counter, UvCache& uv_cache,
                          XyzRefCache& xyz_ref_cache, JacobianProjCache& jacobian_proj_cache) {
  ...
  for (const size_t i : fts) {
    ...
    if (ref_frame.landmark_vec_[i]) {
      ...
      const Position pos =
          seed_ref.keyframe->T_world_cam() * seed_ref.keyframe->getSeedPosInFrame(seed_ref.seed_id);
    ...
    if (!use_distortion_jac &&
        ref_frame.cam()->getType() ==
            Camera::Type::kPinhole) {  // only allow ignoring jacobian for pinhole projection
...
void precomputeJacobiansAndRefPatches(const FramePtr& ref_frame, const UvCache& uv_cache,
                                      const JacobianProjCache& jacobian_proj_cache,
                                      const size_t level, const int patch_size,
                                      const size_t nr_features, bool estimate_alpha,
                                      bool estimate_beta, size_t& feature_counter,
  ...
  for (size_t i = 0; i < nr_features; ++i, ++feature_counter) {
    ...
    for (int y = 0; y < patch_size_wb; ++y) {
      ...
      for (int x = 0; x < patch_size_wb; ++x, ++r, ++pixel_counter) {
        ...
        interp_patch_array[pixel_counter] =
            wtl * r[0] + wtr * r[1] + wbl * r[stride] + wbr * r[stride + 1];
      }
    }
    ...
    for (int y = 0; y < patch_size; ++y) {
      for (int x = 0; x < patch_size; ++x, ++pixel_counter) {
        ...
        const FloatType dx =
            0.5f * (interp_patch_array[offset_center + 1] - interp_patch_array[offset_center - 1]);
...
void computeResidualsOfFrame(const FramePtr& cur_frame, const size_t level, const int patch_size,
                             const size_t nr_features, const Transformation& T_cur_ref,
                             const float alpha, const float beta,
                             const RefPatchCache& ref_patch_cache, const XyzRefCache& xyz_ref_cache,
                             size_t& feature_counter, std::vector<Vector2d>* match_pxs,
                             ResidualCache& residual_cache, VisibilityMask& visibility_mask) {
  ...
  for (size_t i = 0; i < nr_features; ++i, ++feature_counter) {
    ...
    if (cur_frame->cam()->getType() == vk::cameras::CameraGeometryBase::Type::kPinhole &&
        xyz_cur.z() < 0.0) {
    ...
    if (u_tl < 0.0 || v_tl < 0.0 || u_tl + patch_size + 2.0 >= cur_img.cols ||
    ...
    for (int y = 0; y < patch_size; ++y) {
      ...
      for (int x = 0; x < patch_size; ++x, ++pixel_counter, ++cur_img_ptr) {
        ...
        FloatType intensity_cur = wtl * cur_img_ptr[0] + wtr * cur_img_ptr[1] +
        ...
        // for camera control:
        total_res +=
            intensity_cur - static_cast<float>(ref_patch_cache(pixel_counter, feature_counter));
...
FloatType computeHessianAndGradient(const JacobianCache& jacobian_cache,
                                    const ResidualCache& residual_cache,
                                    const VisibilityMask& visibility_mask, const float weight_scale,
  ...
  for (size_t i = 0; i < mask_size; ++i) {
    if (visibility_mask(i) == true) {
      ...
      for (size_t j = 0; j < patch_area; ++j) {
        ...
        // Robustification.
        float weight = 1.0;
        if (weight_function) weight = weight_function->weight(res / weight_scale);
...
float computeResidualHessianGradient(const FramePtr& cur_frame, const size_t level,
                                     const int patch_size, const size_t nr_features,
                                     const Transformation& T_cur_ref, const float alpha,
                                     const float beta, const RefPatchCache& ref_patch_cache,
                                     const XyzRefCache& xyz_ref_cache,
                                     const JacobianCache& jacobian_cache, const float weight_scale,
                                     const vk::solver::WeightFunctionPtr& weight_function,
                                     SparseImgAlign::HessianMatrix* H,
                                     SparseImgAlign::GradientVector* g, size_t& feature_counter) {
  const cv::Mat& cur_img = cur_frame->img_pyr_.at(level);
  const int stride = cur_img.step;
