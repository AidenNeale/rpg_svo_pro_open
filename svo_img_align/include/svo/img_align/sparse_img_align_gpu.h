...
struct HostCacheHandler {
  ...
  inline void push_xyz(const float& x, const float& y, const float& z) {
    xyz_ref_cache.push_back(
        {static_cast<FloatTypeGpu>(x), static_cast<FloatTypeGpu>(y), static_cast<FloatTypeGpu>(z)});
  }
...
class SparseImgAlignGpu : public SparseImgAlignBase {
 ...
 public:
  SparseImgAlignGpu(SolverOptions optimization_options, SparseImgAlignOptions options);

  void setPatchSizeSideEffects() { gpu_cache_.setPatchArea(patch_area_); }

  size_t run(const FrameBundle::Ptr& ref_frames, const FrameBundle::Ptr& cur_frames);

  /**
   * @brief getMedianDisparity returns the median disparty for all features between the current
   * frame and the reference frame. The value is updated when calling
   * sparse_img_align_device_utils::computeDisparity() (see .cpp file)
   */
 ...
 protected:
  ...
  double evaluateError(const SparseImgAlignState& state, HessianMatrix* H, GradientVector* g);
};
...
void extractFeaturesSubset(
    ...
    size_t& nr_fts_extracted, HostCacheHandler& host_cache);
