...
StereoTriangulation::StereoTriangulation(const StereoTriangulationOptions& options,
...
void StereoTriangulation::compute(const FramePtr& frame0, const FramePtr& frame1) {
  ...
  feature_detector_->detect(frame0->img_pyr_, frame0->getMask(), max_n_features, new_px, new_scores,
  ...
  for (const size_t& i_ref : indices) {
    ...
    Matcher::MatchResult res =
        matcher.findEpipolarMatchDirect(*frame0, *frame1, T_f1f0, ref_ftr, options_.mean_depth_inv,
                                        options_.min_depth_inv, options_.max_depth_inv, depth);

    if (res == Matcher::MatchResult::kSuccess) {
      const Position xyz_world =
          frame0->T_world_cam() * (frame0->f_vec_.col(static_cast<int>(i_ref)) * depth);
