...
PreintegratedImuMeasurement::PreintegratedImuMeasurement(const Eigen::Vector3d& omega_bias,
...
ImuHandler::ImuHandler(const ImuCalibration& imu_calib, const ImuInitialization& imu_init,
...
bool ImuHandler::getMeasurements(const double old_cam_timestamp, const double new_cam_timestamp,
...
bool ImuHandler::loadImuMeasurementsFromCsvFile(const std::string& filename) {
  ...
  while (vk::readNextLine(7, csv_file_stream, &fields)) {
    ...
    const ImuMeasurement m(static_cast<double>(stamp_ns) / 1e9, omega,
...
bool ImuHandler::getAngularVelocity(double timestamp, Eigen::Vector3d& omega) const {
  ...
  for (ImuMeasurements::const_iterator it = measurements_.begin(); it != measurements_.end();
       ++it) {
...
bool ImuHandler::waitTill(const double img_timestamp_sec, const double timeout_sec) {
  vk::Timer wait_time;
  wait_time.start();
