...
bool saveMap(const MapPtr& map, const std::string& save_dir, const std::string& map_name) {
  std::string filename = save_dir + "/" + map_name;
  std::cout << "save map to file " << filename << std::endl;
  YAML::Emitter out;
  out << YAML::BeginMap << YAML::Key << "frames" << YAML::Value << YAML::BeginSeq;
  ...
  for (const auto& keyval : map->keyframes_) {
    ...
    out << YAML::BeginMap << YAML::Key << "frame_id" << YAML::Value << frame->id() << YAML::Key
        ...
        << YAML::Flow << YAML::BeginSeq << t.x() << t.y() << t.z() << q.x() << q.y() << q.z()
        << q.w()
        << YAML::EndSeq
...
bool loadMap(const std::string& load_dir, MapPtr& map) {
  ...
  */
  ...
  for (YAML::const_iterator it = data["frames"].begin(); it != data["frames"].end(); ++it) {
    YAML::Node frame = *it;
    Vector3d t_wc(frame["T_world_cam"][0].as<double>(), frame["T_world_cam"][1].as<double>(),
                  frame["T_world_cam"][2].as<double>());
    Eigen::Quaterniond q_wc(
        frame["T_world_cam"][6].as<double>(), frame["T_world_cam"][3].as<double>(),
        frame["T_world_cam"][4].as<double>(), frame["T_world_cam"][5].as<double>());

    FramePtr f =
        std::make_shared<Frame>(frame["frame_id"].as<int>(), frame["timestamp"].as<double>(), cam,
  ...
  for (YAML::const_iterator it = data["points"].begin(); it != data["points"].end(); ++it) {
    YAML::Node point = *it;
    PointPtr p =
        std::make_shared<Point>(point["point_id"].as<int>(),
