...
FrameHandlerMono::FrameHandlerMono(const BaseOptions& base_options,
...
UpdateResult FrameHandlerMono::processFrame() {
  ...
  if (depth_filter_->options_.extra_map_points) {
    ...
    feature_detection_utils::mergeGrids(map_point_grid,
...
UpdateResult FrameHandlerMono::relocalizeFrame(const Transformation& /*T_cur_ref*/,
...
const FramePtr& FrameHandlerMono::newFrame() const { return new_frames_->frames_[0]; }

const FramePtr& FrameHandlerMono::lastFrameUnsafe() const { return last_frames_->frames_[0]; }

bool FrameHandlerMono::haveLastFrame() const { return (last_frames_ != nullptr); }
