...
inline double distanceFirstTwoKeyframes(svo::Map& map) {
  ...
  double dist =
      (first_kf->T_world_imu().getPosition() - second_kf->T_world_imu().getPosition()).norm();
  return dist;
}
}  // namespace
...
const std::unordered_map<svo::TrackingQuality, std::string, EnumClassHash> kTrackingQualityName{
    {TrackingQuality::kInsufficient, "Insufficient"},
    {TrackingQuality::kBad, "Bad"},
    {TrackingQuality::kGood, "Good"}};

const std::unordered_map<svo::UpdateResult, std::string, EnumClassHash> kUpdateResultName{
    {UpdateResult::kDefault, "Default"},
    {UpdateResult::kKeyframe, "KF"},
    {UpdateResult::kFailure, "Failure"}};

FrameHandlerBase::FrameHandlerBase(const BaseOptions& base_options,
  ...
  sparse_img_align_.reset(
      new SparseImgAlign(SparseImgAlign::getDefaultSolverOptions(), img_align_options));
...
bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat>& imgs, const uint64_t timestamp) {
  if (last_frames_) {
    ...
    if (options_.trace_statistics) bundle_adjustment_->setPerformanceMonitor(options_.trace_dir);
  }
  if (options_.trace_statistics) {
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr& frame_bundle) {
  ...
  if (global_map_ && imu_handler_ && last_frames_) {
    ...
    if (imu_handler_->waitTill(cur_t_sec)) {
      imu_handler_->getMeasurements(last_t_sec, cur_t_sec, false, imu_meas_since_last);
    }
  ...
  if (bundle_adjustment_) {
    ...
    if (bundle_adjustment_->getNumFrames() > 0 && have_motion_prior_) {
      bundle_adjustment_->getLatestSpeedBiasPose(&speed_bias_backend_latest_, &T_WS_backend_latest_,
                                                 &timestamp_backend_latest_);
  ...
  if (lc_ && last_frames_ && last_frames_->isKeyframe()) {
    if (!bundle_adjustment_ || (backend_scale_stable && backend_scale_initialized_)) {
...
void FrameHandlerBase::setInitialPose(const FrameBundlePtr& frame_bundle) const {
  if (have_rotation_prior_) {
    ...
    if (std::fabs(z.dot(p)) > std::fabs(z.dot(p_alternative))) p = p_alternative;
...
size_t FrameHandlerBase::sparseImageAlignment() {
  ...
  if (have_motion_prior_) {
    ...
    sparse_img_align_->setWeightedPrior(T_newimu_lastimu_prior_, 0.0, 0.0,
                                        options_.img_align_prior_lambda_rot, prior_trans, 0.0, 0.0);
  }
...
size_t FrameHandlerBase::projectMapInFrame() {
  ...
  for (size_t camera_idx = 0; camera_idx < cams_->numCameras(); ++camera_idx) {
    ...
    map_->getClosestNKeyframesWithOverlap(new_frames_->at(camera_idx),
                                          cur_reprojector->options_.max_n_kfs,
                                          &overlap_kfs_.at(camera_idx));
#ifdef SVO_GLOBAL_MAP
    if (!isInRecovery() && cur_reprojector->options_.use_kfs_from_global_map && global_map_) {
      std::vector<FramePtr> overlap_kfs_global;
      global_map_->getOverlapKeyframesMaxN(*new_frames_->at(camera_idx),
  ...
  if (options_.use_async_reprojectors && cams_->numCameras() > 1) {
    ...
    for (size_t i = 0; i < reprojector_workers.size(); ++i) reprojector_workers[i].get();
  } else {
    for (size_t camera_idx = 0; camera_idx < cams_->numCameras(); ++camera_idx) {
      reprojectors_.at(camera_idx)
          ->reprojectFrames(new_frames_->at(camera_idx), overlap_kfs_.at(camera_idx),
                            trash_points.at(camera_idx));
    }
  }
...
size_t FrameHandlerBase::optimizePose() {
  ...
  // coordinates is not far off the position of the feature points within the frame. The
  // optimization is done for all points in the same time, hence optimizing frame pose.
  if (options_.trace_statistics) {
...
void FrameHandlerBase::optimizeStructure(const FrameBundle::Ptr& frames, int max_n_pts,
  ...
  if (max_n_pts == 0) return;  // don't return if max_n_pts == -1, this means we optimize ALL points
  ...
  for (const FramePtr& frame : frames->frames_) {
    ...
    if (frame->cam()->getType() == Camera::Type::kOmni) optimize_on_sphere = true;
    std::deque<PointPtr> pts;
    for (size_t i = 0; i < frame->num_features_; ++i) {
      if (frame->landmark_vec_[i] == nullptr || isEdgelet(frame->type_vec_[i])) continue;
  ...
  for (size_t i = 0; i < frame->num_features_; ++i) {
    if (frame->landmark_vec_[i]) {
      ...
      if (point == nullptr) {
        ...
        Position xyz_world =
            ref.keyframe->T_world_cam() * ref.keyframe->getSeedPosInFrame(ref.seed_id);
...
void FrameHandlerBase::setTrackingQuality(const size_t num_observations) {
  ...
  if (!last_frames_->isKeyframe() && feature_drop > options_.quality_max_fts_drop) {
...
bool FrameHandlerBase::needNewKf(const Transformation&) {
  ...
  for (const auto& kf : visible_kfs) {
    ...
    const double a = Quaternion::log(new_frames_->at(0)->T_f_w_.getRotation() *
