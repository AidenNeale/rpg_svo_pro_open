...
size_t PoseOptimizer::run(const FrameBundle::Ptr& frame_bundle, double reproj_thresh_px) {
  ...
  if (ofs_reproj_errors_.is_open()) {
    for (auto i : start_errors) ofs_reproj_errors_ << i * error_scale << ", ";
    ofs_reproj_errors_ << std::endl;
    for (auto i : final_errors) ofs_reproj_errors_ << i * error_scale << ", ";
    ofs_reproj_errors_ << std::endl;
  }
...
double PoseOptimizer::evaluateErrorImpl(const Transformation& T_imu_world, HessianMatrix* H,
  ...
  if (unwhitened_errors) unwhitened_errors->reserve(frame_bundle_->numFeatures());
      ...
      if (isEdgelet(frame->type_vec_[i])) {
        ...
        if (err_type_ == ErrorType::kUnitPlane)
          ...
          pose_optimizer_utils::calculateEdgeletResidualImagePlane(
              ...
              *frame->cam(), measurement_sigma, robust_weight_, &unwhitened_error, &chi2_error, H,
              g);
...
void PoseOptimizer::removeOutliers(const double reproj_err_threshold, Frame* frame,
  ...
  static double threshold_bearing_diff =
      std::fabs(2 * std::sin(0.5 * frame->getAngleError(reproj_err_threshold)));
  ...
  for (size_t i = 0; i < frame->num_features_; ++i) {
    ...
    if (isEdgelet(frame->type_vec_[i])) {
      if (err_type_ == ErrorType::kUnitPlane) {
        pose_optimizer_utils::calculateEdgeletResidualUnitPlane(
            frame->f_vec_.col(i), xyz_world, frame->grad_vec_.col(i), T_imu_world, T_cam_imu, 0.0,
            robust_weight_, &unwhitened_error, &chi2_error, nullptr, nullptr);
      } else if (err_type_ == ErrorType::kBearingVectorDiff) {
        pose_optimizer_utils::calculateEdgeletResidualBearingVectorDiff(
            frame->px_vec_.col(i), frame->f_vec_.col(i), xyz_world, frame->grad_vec_.col(i),
            T_imu_world, T_cam_imu, *frame->cam(), 0.0, robust_weight_, &unwhitened_error,
            &chi2_error, nullptr, nullptr);
      } else if (err_type_ == ErrorType::kImagePlane) {
        pose_optimizer_utils::calculateEdgeletResidualImagePlane(
            frame->px_vec_.col(i), xyz_world, frame->grad_vec_.col(i), T_imu_world, T_cam_imu,
            *frame->cam(), 0.0, robust_weight_, &unwhitened_error, &chi2_error, nullptr, nullptr);
      }
    } else {
      if (err_type_ == ErrorType::kUnitPlane) {
        pose_optimizer_utils::calculateFeatureResidualUnitPlane(
            frame->f_vec_.col(i), xyz_world, T_imu_world, T_cam_imu, 0.0, robust_weight_,
        ...
        pose_optimizer_utils::calculateFeatureResidualImagePlane(
            frame->px_vec_.col(i), xyz_world, T_imu_world, T_cam_imu, *frame->cam(), 0.0,
            robust_weight_, &unwhitened_error, &chi2_error, nullptr, nullptr);
      }
    }
...
void PoseOptimizer::update(const State& T_imuold_world, const UpdateVector& dx,
...
void PoseOptimizer::applyPrior(const State& T_cur_from_world) {
  if (iter_ == 0) {
    ...
    double H_max_diag = 0;
    // double tau = 1e-4;
    for (size_t j = 3; j < 6; ++j) H_max_diag = std::max(H_max_diag, std::fabs(H_(j, j)));
...
void calculateFeatureResidualImagePlane(
    ...
    const Transformation& T_imu_world, const Transformation& T_cam_imu, const svo::Camera& cam,
...
void calculateEdgeletResidualImagePlane(
    const Eigen::Ref<const Keypoint>& px, const Position& xyz_in_world,
    const Eigen::Ref<const GradientVector>& grad, const Transformation& T_imu_world,
    const Transformation& T_cam_imu, const svo::Camera& cam, double measurement_sigma,
    const PoseOptimizer::RobustWeightFunction& robust_weight, double* unwhitened_error,
    double* chi2_error, PoseOptimizer::HessianMatrix* H, PoseOptimizer::GradientVector* g) {
  const Vector3d xyz_in_imu(T_imu_world * xyz_in_world);
  const Vector3d xyz_in_cam(T_cam_imu * xyz_in_imu);
