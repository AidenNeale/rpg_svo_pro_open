...
FrameHandlerArray::FrameHandlerArray(const BaseOptions& base_options,
...
void FrameHandlerArray::addImages(const std::vector<cv::Mat>& images, const uint64_t timestamp) {
  // TODO: deprecated
...
UpdateResult FrameHandlerArray::processSecondFrame() {
  ...
  for (const FramePtr& frame : initializer_->frames_ref_->frames_) {
    frame->setKeyframe();
    map_->addKeyframe(frame, bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
  }
  ...
  for (const FramePtr& frame : new_frames_->frames_) {
    ...
    depth_filter_->addKeyframe(frame, depth_median_, 0.5 * depth_min_, depth_median_ * 1.5);
    map_->addKeyframe(frame, bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
  }
...
UpdateResult FrameHandlerArray::processFrame() {
  ...
  if (n_tracked_features < options_.quality_min_fts) return UpdateResult::kFailure;
  ...
  if (tracking_quality_ == TrackingQuality::kInsufficient) return UpdateResult::kFailure;
...
UpdateResult FrameHandlerArray::makeKeyframe(const size_t camera_id) {
  ...
  depth_filter_->addKeyframe(new_frames_->at(camera_id), depth_median, 0.5 * depth_min,
                             depth_median * 1.5);
