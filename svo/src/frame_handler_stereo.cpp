...
FrameHandlerStereo::FrameHandlerStereo(
    const BaseOptions& base_options, const DepthFilterOptions& depth_filter_options,
    const DetectorOptions& feature_detector_options, const InitializationOptions& init_options,
    const StereoTriangulationOptions& stereo_options, const ReprojectorOptions& reprojector_options,
    const FeatureTrackerOptions& tracker_options, const CameraBundle::Ptr& stereo_camera)
  ...
  stereo_triangulation_.reset(new StereoTriangulation(
      stereo_options,
      feature_detection_utils::makeDetector(feature_detector_options, cams_->getCameraShared(0))));
}
...
void FrameHandlerStereo::addImages(const cv::Mat& img_left, const cv::Mat& img_right,
...
UpdateResult FrameHandlerStereo::processFirstFrame() {
  ...
  map_->addKeyframe(new_frames_->at(0), bundle_adjustment_type_ == BundleAdjustmentType::kCeres);
  new_frames_->at(1)->setKeyframe();
  map_->addKeyframe(new_frames_->at(1), bundle_adjustment_type_ == BundleAdjustmentType::kCeres);

  frame_utils::getSceneDepth(new_frames_->at(0), depth_median_, depth_min_, depth_max_);
  depth_filter_->addKeyframe(new_frames_->at(0), depth_median_, 0.5 * depth_min_,
                             depth_median_ * 1.5);
...
UpdateResult FrameHandlerStereo::makeKeyframe() {
  ...
  depth_filter_->addKeyframe(new_frames_->at(kf_id), depth_median_, 0.5 * depth_min_,
                             depth_median_ * 1.5);
