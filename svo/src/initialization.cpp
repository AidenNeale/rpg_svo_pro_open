...
bool AbstractInitialization::trackFeaturesAndCheckDisparity(const FrameBundlePtr& frames) {
  ...
  tracker_->getNumTrackedAndDisparityPerFrame(options_.init_disparity_pivot_ratio, &num_tracked,
                                              &disparity);
...
InitResult HomographyInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
  ...
  if (!trackFeaturesAndCheckDisparity(frames_cur)) return InitResult::kTracking;
  ...
  // Compute model
  const vk::Homography H_cur_ref =
      vk::estimateHomography(f_cur, f_ref, frames_ref_->at(0)->getErrorMultiplier(),
...
InitResult TwoPointInit::addFrameBundle(const FrameBundlePtr& frames_cur) {
  ...
  const FrameBundlePtr frames_ref = tracker_->getOldestFrameInTrack(0);
  FeatureMatches matches_cur_ref;
  feature_tracking_utils::getFeatureMatches(*frames_cur->at(0), *frames_ref->at(0),
                                            &matches_cur_ref);

  // Create vector of bearing vectors
  BearingVectors f_cur;
  BearingVectors f_ref;
  initialization_utils::copyBearingVectors(*frames_cur->at(0), *frames_ref->at(0), matches_cur_ref,
                                           &f_cur, &f_ref);
  ...
  // enough inliers?
  if (ransac.inliers_.size() < options_.init_min_inliers) return InitResult::kNoKeyframe;
...
InitResult ArrayInitGeometric::addFrameBundle(const FrameBundlePtr& frames_cur) {
#ifdef SVO_USE_OPENGV
  /*
  ...
  std::cos(frames_cur->at(0)->getAngleError(options_.reproj_error_thresh)); ransac.sac_model_ =
  problem_ptr; ransac.threshold_ = inlier_threshold; ransac.max_iterations_ = 500;
  ransac.probability_ = 0.995;
...
InitResult ArrayInitOptimization::addFrameBundle(const FrameBundlePtr& frames_cur) {
#ifdef SVO_USE_GTSAM
  ...
  graph.push_back(gtsam::PriorFactor<gtsam::Pose3>(gtsam::Symbol('x', 0), pose_ref,
  ...
  graph.push_back(gtsam::BetweenFactor<gtsam::Pose3>(gtsam::Symbol('x', 0), gtsam::Symbol('x', 1),
  ...
  int point_id_ref = 0;
  typedef gtsam::GenericProjectionFactor<gtsam::Pose3, gtsam::Point3, gtsam::Cal3_S2>
  ProjectionFactor; for(const FramePtr& frame : frames_ref_->frames_)
  {
  ...
  gtsam::Values result = gtsam::LevenbergMarquardtOptimizer(graph, initial_estimate,
  params).optimize();
...
bool triangulateAndInitializePoints(const FramePtr& frame_cur, const FramePtr& frame_ref,
  ...
  initialization_utils::triangulatePoints(*frame_cur, *frame_ref, T_cur_ref, reprojection_threshold,
...
void triangulatePoints(const Frame& frame_cur, const Frame& frame_ref,
  ...
  const double angle_threshold = frame_cur.getAngleError(reprojection_threshold);
  for (size_t i = 0; i < matches_cur_ref.size(); ++i) {
    ...
    const Position xyz_in_cur = vk::triangulateFeatureNonLin(T_cur_ref.getRotationMatrix(),
    ...
    if (e1 > angle_threshold || e2 > angle_threshold ||
        (frame_cur.cam()->getType() == Camera::Type::kPinhole && xyz_in_cur.z() < 0.0)) {
  ...
  if (!outliers.empty()) {
    svo::common::container_helpers::eraseIndicesFromVector(outliers, &matches_cur_ref);
  }
}

void rescaleAndInitializePoints(const FramePtr& frame_cur, const FramePtr& frame_ref,
...
AbstractInitialization::UniquePtr makeInitializer(const InitializationOptions& init_options,
  ...
  switch (init_options.init_type) {
    ...
    case InitializerType::kArrayGeometric:
      initializer.reset(
          new ArrayInitGeometric(init_options, tracker_options, detector_options, cams));
      break;
...
void copyBearingVectors(const Frame& frame_cur, const Frame& frame_ref,
                        const AbstractInitialization::FeatureMatches& matches_cur_ref,
