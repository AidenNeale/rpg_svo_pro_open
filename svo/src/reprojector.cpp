...
void Reprojector::reprojectFrames(const FramePtr& cur_frame,
  ...
  if (!grid_) {
    grid_.reset(new OccupandyGrid2D(
        options_.cell_size,
        OccupandyGrid2D::getNCell(cur_frame->cam()->imageWidth(), options_.cell_size),
        OccupandyGrid2D::getNCell(cur_frame->cam()->imageHeight(), options_.cell_size)));
  }
  grid_->reset();
  stats_.reset();
  ...
  if (options_.use_kfs_from_global_map && options_.max_n_global_kfs > 0) {
    if (!fixed_landmark_grid_) {
      fixed_landmark_grid_.reset(new OccupandyGrid2D(
          ...
          OccupandyGrid2D::getNCell(cur_frame->cam()->imageWidth(), options_.fixed_lm_grid_size),
          OccupandyGrid2D::getNCell(cur_frame->cam()->imageHeight(), options_.fixed_lm_grid_size)));
    }
    ...
    for (const auto& c : candidates_) {
      ...
      old_proj_info.emplace_back(
          std::make_pair(pt, SucFailN{pt->n_succeeded_reproj_, pt->n_failed_reproj_}));
    }
    reprojector_utils::sortCandidatesByNumObs(candidates_);
    reprojector_utils::matchCandidates(cur_frame, options_.max_fixed_landmarks,
  ...
  for (const FramePtr& ref_frame : visible_kfs) {
    ...
    for (size_t i = 0; i < ref_frame->num_features_; ++i) {
      ...
      if (ref_frame->landmark_vec_[i] == nullptr || type == FeatureType::kOutlier ||
      ...
      if (point->last_projected_kf_id_.at(camera_index_) == cur_frame->id_) continue;
      point->last_projected_kf_id_[camera_index_] = cur_frame->id_;
...
void sortCandidatesByReprojStats(Reprojector::Candidates& candidates) {
  std::sort(candidates.begin(), candidates.end(),
            [](const Reprojector::Candidate& lhs, const Reprojector::Candidate& rhs) {
              if (lhs.type > rhs.type || (lhs.type == rhs.type && lhs.n_reproj > rhs.n_reproj) ||
  ...
  std::sort(candidates.begin(), candidates.end(),
            [](const Reprojector::Candidate& lhs, const Reprojector::Candidate& rhs) {
              if (lhs.n_obs > rhs.n_obs ||
                  ...
                  (lhs.n_obs == rhs.n_obs && lhs.n_reproj == rhs.n_reproj && lhs.score > rhs.score))
                return true;
              return false;
            });
}

void matchCandidates(const FramePtr& frame, const size_t max_n_features_per_frame,
                     const bool affine_est_offset, const bool affine_est_gain,
                     Reprojector::Candidates& candidates, OccupandyGrid2D& grid,
                     Reprojector::Statistics& stats, const double seed_sigma2_thresh) {
  Matcher matcher;
  ...
  for (Reprojector::Candidate& candidate : candidates) {
    ...
    if (matchCandidate(frame, candidate, matcher, feature_wrapper, seed_sigma2_thresh)) {
      ++stats.n_matches;
      ++frame->num_features_;
...
bool matchCandidate(const FramePtr& frame, Reprojector::Candidate& c, Matcher& matcher,
                    FeatureWrapper& feature, const double seed_sigma2_thresh) {
  ...
  if (c.ref_frame->landmark_vec_.at(c.ref_index) == nullptr) {
    ...
    } else if (isUnconvergedCornerEdgeletSeed(c.type) || isUnconvergedMapPointSeed(c.type)) {
      if (!depth_filter_utils::updateSeed(*frame, *c.ref_frame, c.ref_index, matcher,
  ...
  frame->in_ba_graph_vec_[frame->numFeatures()] = c.ref_frame->in_ba_graph_vec_[c.ref_index];
  return true;
}

bool getCandidate(const FramePtr& cur_frame, const FramePtr& ref_frame, const size_t& ref_index,
...
bool projectPointAndCheckVisibility(const FramePtr& frame, const Eigen::Vector3d& xyz,
  ...
  if (!frame->cam()->isKeypointVisibleWithMargin(pxi, kPatchSize)) return false;
  return true;
}

void setGridCellsOccupied(const Reprojector::Candidates& candidates, OccupandyGrid2D& grid) {
...
void reprojectMapPoints(const FramePtr& frame, const std::vector<FramePtr>& overlap_kfs,
                        const ReprojectorOptions& options, OccupandyGrid2D* grid) {
