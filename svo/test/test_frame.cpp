...
struct BearingVectorFunctor : public aslam::common::NumDiffFunctor<3, 6> {
  BearingVectorFunctor(const Transformation& T_cam_imu, const Eigen::Vector3d& p_in_imu)
...
struct UnitPlaneFunctor : public aslam::common::NumDiffFunctor<2, 6> {
  UnitPlaneFunctor(const Transformation& T_cam_imu, const Eigen::Vector3d& p_in_imu,
...
struct ImagePlaneFunctor : public aslam::common::NumDiffFunctor<2, 6> {
  ImagePlaneFunctor(std::shared_ptr<const svo::Camera> cam, const Transformation& T_cam_imu,
  ...
  virtual bool functional(const typename NumDiffFunctor::InputType& x,
    ...
    Transformation T_update = Transformation::exp(x);
    const Eigen::Vector3d p_in_imu = T_update * p_in_imu_;
    if (Jout) Frame::jacobian_xyz2image_imu(*cam_, T_cam_imu_, p_in_imu, *Jout);
    const Eigen::Vector3d p_in_cam = T_cam_imu_ * p_in_imu;
...
TEST_F(SVOFrameTest, testBearingVectorJacobian) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    TEST_JACOBIAN_FINITE_DIFFERENCE(BearingVectorFunctor, input, 1e-6, 1e-5, T_cam_imu, p_in_imu);
  }
}
...
TEST_F(SVOFrameTest, testUnitPlaneJacobian) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    f_in_imu.normalize();
    TEST_JACOBIAN_FINITE_DIFFERENCE(UnitPlaneFunctor, input, 1e-7, 1e-3, T_cam_imu, p_in_imu,
                                    f_in_imu);
  }
}
...
TEST_F(SVOFrameTest, testOmniImageJacobian) {
  ...
  inverse_polynomial << 142.7468, 104.8486, 7.3973, 17.4581, 12.6308, -4.3751, 6.9093, 10.9703,
      -0.6053, -3.9119, -1.0675, 0.0;
  const Eigen::Vector2d mask_relative_radii(0.0, 1.0);

  svo::CameraPtr omni_cam = svo::CameraPtr(new vk::cameras::OmniGeometry(
      640, 480, polynomial, principal_point, distortion, inverse_polynomial, mask_relative_radii));
  ...
  for (int i = 0; i < 10; i++) {
    ...
    while (true) {
      p_in_imu = test_utils::generateRandomPoint(1, 10);
      Eigen::Vector2d kp;
      if (omni_cam->project3(T_cam_imu * p_in_imu, &kp).isKeypointVisible()) break;
    }
    // on image plane, should be 2-order higher than unit plane error
    TEST_JACOBIAN_FINITE_DIFFERENCE(ImagePlaneFunctor, input, 1e-4, 5 * 1e-2, omni_cam, T_cam_imu,
                                    p_in_imu);
  }
}
...
TEST_F(SVOFrameTest, testPinholeImageJacobian) {
  ...
  svo::CameraPtr pinhole_cam = vk::cameras::factory::makePinholeCamera(pinhole_intrin, 752, 480);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    while (true) {
      p_in_imu = test_utils::generateRandomPoint(1, 10);
      Eigen::Vector2d kp;
      if (pinhole_cam->project3(T_cam_imu * p_in_imu, &kp).isKeypointVisible()) break;
    }
    // on image plane, should be 2-order higher than unit plane error
    TEST_JACOBIAN_FINITE_DIFFERENCE(ImagePlaneFunctor, input, 1e-4, 5 * 1e-2, pinhole_cam,
                                    T_cam_imu, p_in_imu);
  }
