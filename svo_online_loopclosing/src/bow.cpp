...
void extractBoWFeaturesFromImage(const cv::Mat& image, std::vector<cv::Point2f>* keypoints_pt2f,
                                 std::vector<cv::Mat>* feature) {
  feature->clear();
...
OrbVocabulary createVoc(const std::vector<std::vector<cv::Mat> >& features,
                        const std::string& save_path, const std::string& voc_name, const int k,
                        const int L, const WeightingType& weight, const ScoringType& score) {
...
void createBOW(const std::vector<cv::Mat>& feature, const OrbVocabulary& voc, BowVector* v,
...
double compareBOWs(const BowVector& v1, const BowVector& v2, const OrbVocabulary& voc) {
  double score = voc.score(v1, v2);

  return score;
}

void getNodeID(const std::vector<cv::Mat>& feature, const OrbVocabulary& voc, const int levelup,
...
void extractFeaturesFromSVOKeypoints(const cv::Mat& image, std::vector<cv::Point3f>* svo_landmarks,
  ...
  for (unsigned int i = 0; i < keypoints.size(); i++) {
    ...
    int pos =
        std::find(old_svo_keypoints.begin() + i, old_svo_keypoints.end(), svo_keypoints->back()) -
