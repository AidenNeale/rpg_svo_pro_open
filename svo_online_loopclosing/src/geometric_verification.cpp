...
void featureMatchingFast(const std::vector<cv::Mat>& feature_vec1,
                         ...
                         const std::vector<cv::Mat>& feature_vec4, cv::Mat& svo_feature_mat1,
                         cv::Mat& svo_feature_mat2, const std::vector<int>& node_id1,
                         const std::vector<int>& node_id2, const std::vector<int>& node_id3,
  ...
  for (unsigned int i = 0; i < feature_vec1_size; i++) {
    ...
    while ((iter = std::find(iter, node_id2.end(), node_id1[i])) != node_id2.end()) {
      ...
      double hamming_dist = cv::norm(feature_vec1[i], feature_vec2[index], cv::NORM_HAMMING);
  ...
  for (unsigned int i = 0; i < feature_vec4_size; i++) {
    ...
    while ((iter = std::find(iter, node_id3.end(), node_id4[i])) != node_id3.end()) {
      ...
      double hamming_dist_svo = cv::norm(feature_vec4[i], feature_vec3[index], cv::NORM_HAMMING);
    ...
    if (matched_index_svo != -1) {
      ...
      match_indices->col(num_matches - 1) << matched_index_svo + feature_vec1_size,
          i + feature_vec2_size;
    }
  }
  //  std::cout<<"================== Matching Finished
  //  ================"<<std::endl;
  std::cout << "Total matches " << num_matches;
  match_indices->conservativeResize(2, num_matches);
}

void geometricVerification(
    const vector<cv::Point2f>& keypoints1, const vector<cv::Point2f>& keypoints2,
    const BearingVecs& svo_bearingvector1, const BearingVecs& svo_bearingvector2,
    const Eigen::MatrixXd& match_indices, const cv::Mat& K, const Eigen::VectorXd& dist_par,
    const int& bow_featurevec1_size, const int& bow_featurevec2_size, const bool& use_open_gv,
    cv::Mat* inliers, vector<cv::Point2f>* keypoints_matched1,
    vector<cv::Point2f>* keypoints_matched2, vector<cv::Point2f>* keypoints_matched1_udist,
    vector<cv::Point2f>* keypoints_matched2_udist, Eigen::MatrixXd* T_rel) {
  ...
  vector<cv::Point2f>* keypoints_matched1_norm_udist =
      new vector<cv::Point2f>(match_indices.cols());  // normalised and undistorted matches
  ...
  cv::undistortPoints(*keypoints_matched1, *keypoints_matched1_norm_udist, K, D, cv::noArray(),
                      cv::noArray());
  cv::undistortPoints(*keypoints_matched2, *keypoints_matched2_norm_udist, K, D, cv::noArray(),
                      cv::noArray());
  ...
  if (use_open_gv) {
    ...
    for (int i = 0; i < match_indices.cols(); i++) {
      ...
      if (match_indices(0, i) < bow_featurevec1_size &&
        ...
        Eigen::Vector3d bv1 = Eigen::Vector3d(keypoints_matched1_norm_udist->at(i).x,
                                              keypoints_matched1_norm_udist->at(i).y, 1.0);
        bv1 = bv1 / bv1.norm();
        mixed_vec1.push_back(bv1);
        Eigen::Vector3d bv2 = Eigen::Vector3d(keypoints_matched2_norm_udist->at(i).x,
    ...
    for (int i = 0; i < match_indices.cols(); i++) {
      if (match_indices(0, i) >= bow_featurevec1_size &&
          match_indices(1, i) >= bow_featurevec2_size) {
        mixed_vec1.push_back(svo_bearingvector1[match_indices(0, i) - bow_featurevec1_size]);
        mixed_vec2.push_back(svo_bearingvector2[match_indices(1, i) - bow_featurevec2_size]);
      }
    }
    if (mixed_vec1.size() != keypoints_matched1_udist->size()) {
      std::cout << "CHECK: sizes of bearing vec 1 and keypoints vec 1 " << mixed_vec1.size()
                << " and " << keypoints_matched1_udist->size() << std::endl;
      std::cout << "CHECK: sizes of bearing vec 2 and keypoints vec 2 " << mixed_vec2.size()
                << " and " << keypoints_matched2_udist->size() << std::endl;
      throw std::runtime_error(
          "CHECK: sizes of bearing vec 1 and keypoints vec 1 and 2 do not match");
    }
    static double inlier_threshold = 1.0 - std::cos(std::atan(1.5 / (2.0 * K.at<double>(0, 0))) +
                                                    std::atan(1.5 / (2.0 * K.at<double>(1, 1))));
    typedef opengv::sac_problems::relative_pose::CentralRelativePoseSacProblem CentralRelative;
    opengv::relative_pose::CentralRelativeAdapter adapter(mixed_vec1, mixed_vec2);
    ...
    *inliers = 0;
    std::cout << "RANSAC took " << ransac.iterations_ << " iterations.";
    ...
    for (int i = 0; i < match_indices.cols(); i++) {
      keypoints_matched1_udist->at(i).x =
          keypoints_matched1_norm_udist->at(i).x * K.at<double>(0, 0) + K.at<double>(0, 2);
      keypoints_matched1_udist->at(i).y =
          keypoints_matched1_norm_udist->at(i).y * K.at<double>(1, 1) + K.at<double>(1, 2);
      keypoints_matched2_udist->at(i).x =
          keypoints_matched2_norm_udist->at(i).x * K.at<double>(0, 0) + K.at<double>(0, 2);
      keypoints_matched2_udist->at(i).y =
          keypoints_matched2_norm_udist->at(i).y * K.at<double>(1, 1) + K.at<double>(1, 2);
    }
    cv::Mat ematrix;
    ematrix = cv::findEssentialMat(*keypoints_matched1_udist, *keypoints_matched2_udist, K,
                                   cv::RANSAC, 0.995, 1., *inliers);
    //  ematrix = cv::findEssentialMat(*keypoints_matched1_udist,
    //  *keypoints_matched2_udist, K, cv::LMEDS, 0.999, 1.,
    //  *inliers);
    getRelativePose(ematrix, K, *keypoints_matched1_udist, *keypoints_matched2_udist, inliers,
                    T_rel);
...
void getRelativePose(const cv::Mat& eMatrix, const cv::Mat& K,
                     ...
                     const vector<cv::Point2f>& keypoints_matched2, cv::Mat* inliers,
                     Eigen::MatrixXd* T) {
  cv::Mat R, t, mask;
  double focal = (K.at<double>(0, 0) + K.at<double>(1, 1)) / 2;
  cv::Point2d pp(K.at<double>(0, 2), K.at<double>(1, 2));
  int inlierCount =
      cv::recoverPose(eMatrix, keypoints_matched1, keypoints_matched2, K, R, t, *inliers);
  ...
  *T << R.at<double>(0, 0), R.at<double>(0, 1), R.at<double>(0, 2), t.at<double>(0, 0),
      ...
      R.at<double>(2, 0), R.at<double>(2, 1), R.at<double>(2, 2), t.at<double>(2, 0), 0, 0, 0, 1;
}
...
float getScaleCL(const std::vector<cv::Point2f>& keypoints1,
                 ...
                 const std::vector<cv::Point3f>& landmarks2, const std::vector<int>& track_IDs1,
                 const std::vector<int>& track_IDs2, const cv::Mat& cam_pose_current,
                 const cv::Mat& K, const Eigen::MatrixXd& relative_pose,
  ...
  P1 = K * P1;
  cv::Mat P2 = (cv::Mat_<double>(3, 4) << relative_pose(0, 0), relative_pose(0, 1),
                relative_pose(0, 2), relative_pose(0, 3), relative_pose(1, 0), relative_pose(1, 1),
                relative_pose(1, 2), relative_pose(1, 3), relative_pose(2, 0), relative_pose(2, 1),
  ...
  for (unsigned int i = 0; i < landmarks1.size(); i++) {
    ...
    for (unsigned int j = 0; j < landmarks2.size(); j++) {
      if (track_IDs1[i] == track_IDs2[j] && cv::norm(landmarks1[i] - landmarks2[j]) < mindist) {
        minindex = j;
        break;
    ...
    if (minindex > -1) {
      count++;
      cv::Mat svo3DpntHW =
          (cv::Mat_<double>(4, 1) << landmarks1[i].x, landmarks1[i].y, landmarks1[i].z, 1);
      ...
      cv::triangulatePoints(P1, P2, cv::Mat(keypoints1[i]), cv::Mat(keypoints2[minindex]), pnt3DH);
      cv::Point3f pnt3D;
  ...
  if (count == 0 || std::isnan(cumulative_scale) || std::isnan(count)) {
    scale = 1;
    std::cout << "Scale could not be retrieved, use only rotation for pose "
                 "optimisation";
  } else {
...
bool commonLandMarkCheck(const std::vector<int>& track_IDs1, const std::vector<int>& track_IDs2,
                         const double th) {
...
float getScaleMK(const std::vector<cv::Point2f>& keypoints_matched1,
                 ...
                 const std::vector<cv::Point3f>& landmarks1,
                 const std::vector<cv::Point3f>& landmarks2, const Eigen::MatrixXd& match_indices,
                 const cv::Mat& inliers, const cv::Mat& cam_pose, const cv::Mat& K,
                 const Eigen::MatrixXd& relative_pose, const int num_bow_features) {
  float scale2;
  float cumulative_scale = 0;
  int count = 0;
  ...
  P1 = K * P1;
  Eigen::Matrix3d R;
  R << relative_pose(0, 0), relative_pose(0, 1), relative_pose(0, 2), relative_pose(1, 0),
      relative_pose(1, 1), relative_pose(1, 2), relative_pose(2, 0), relative_pose(2, 1),
      relative_pose(2, 2);
  Eigen::Vector3d t;
  ...
  proj_mat << R, t;
  cv::Mat P2 = (cv::Mat_<double>(3, 4) << proj_mat(0, 0), proj_mat(0, 1), proj_mat(0, 2),
                proj_mat(0, 3), proj_mat(1, 0), proj_mat(1, 1), proj_mat(1, 2), proj_mat(1, 3),
  ...
  for (int i = 0; i < match_indices.cols(); i++) {
    ...
    if (inliers.at<bool>(i, 0) == 1 && match_indices(0, i) > num_bow_features) {
      ...
      // Linear triangulation
      cv::Mat pnt3DH(4, 1, CV_64FC4);
      cv::triangulatePoints(P1, P2, cv::Mat(keypoints_matched2[i]), cv::Mat(keypoints_matched1[i]),
                            pnt3DH);
      cv::Point3f pnt3D;
      pnt3D.x = pnt3DH.at<float>(0, 0) / pnt3DH.at<float>(3, 0);
      pnt3D.y = pnt3DH.at<float>(1, 0) / pnt3DH.at<float>(3, 0);
      ...
      // dl
      cv::Mat P_h = (cv::Mat_<double>(4, 1) << landmarks2[match_indices(1, i) - num_bow_features].x,
