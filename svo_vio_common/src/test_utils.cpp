...
// -----------------------------------------------------------------------------
Keypoints generateRandomKeypoints(const uint32_t image_width, const uint32_t image_height,
                                  const uint32_t margin, const uint32_t num_keypoints) {
  if (image_width <= margin + 1u || image_height <= margin + 1u) {
    throw std::invalid_argument("Image size is too small.");
  }

  Keypoints kp(2, num_keypoints);
  for (uint32_t i = 0u; i < num_keypoints; ++i) {
    kp(0, i) = sampleUniformRealDistribution<FloatType>(false, margin, image_width - 1 - margin);
    kp(1, i) = sampleUniformRealDistribution<FloatType>(false, margin, image_height - 1 - margin);
  }
  return kp;
}

// -----------------------------------------------------------------------------
Keypoints generateUniformKeypoints(const uint32_t image_width, const uint32_t image_height,
                                   const uint32_t margin, const uint32_t num_cols) {
  if (image_width <= margin + 1u || image_height <= margin + 1u) {
    throw std::invalid_argument("Image size is too small.");
  }
  ...
  FloatType w = (static_cast<FloatType>(image_width - 0.01) - 2.0 * margin) / (num_cols - 1);
  FloatType h = (static_cast<FloatType>(image_height - 0.01) - 2.0 * margin) / (num_rows - 1);
...
std::tuple<Keypoints, Bearings, Positions> generateRandomVisible3dPoints(
    const Camera& cam, const uint32_t num_points, const uint32_t margin, const FloatType min_depth,
    const FloatType max_depth) {
  Keypoints px = generateRandomKeypoints(cam.imageWidth(), cam.imageHeight(), margin, num_points);
  Bearings f;
...
// -----------------------------------------------------------------------------
FloatType overlappingFieldOfView(const CameraBundle& rig, const uint32_t cam_A,
                                 const uint32_t cam_B) {
  if (cam_A >= rig.getNumCameras() || cam_B >= rig.getNumCameras()) {
