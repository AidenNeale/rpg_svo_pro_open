...
Homography estimateHomography(const Bearings& f_cur, const Bearings& f_ref,
                              const double focal_length, const double reproj_error_thresh,
                              const size_t min_num_inliers) {
  // TODO: too long. split up and write tests

  if (f_cur.cols() != f_ref.cols()) {
    throw std::runtime_error("f_cur and f_ref must have same number of columns");
  }
  ...
  Matrix3d H_cur_ref;
  H_cur_ref << H.at<double>(0, 0), H.at<double>(0, 1), H.at<double>(0, 2), H.at<double>(1, 0),
      H.at<double>(1, 1), H.at<double>(1, 2), H.at<double>(2, 0), H.at<double>(2, 1),
      H.at<double>(2, 2);
  ...
  if (rotations.size() != 4) {
    throw std::runtime_error("decomposeHomographyMat returned " + std::to_string(rotations.size()) +
                             " rotations, expected 4.");
  }
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    d.R_cur_ref << R.at<double>(0, 0), R.at<double>(0, 1), R.at<double>(0, 2), R.at<double>(1, 0),
  ...
  std::sort(decomp.begin(), decomp.end(),
            [&](const Homography& lhs, const Homography& rhs) { return lhs.score > rhs.score; });
  decomp.resize(2);
