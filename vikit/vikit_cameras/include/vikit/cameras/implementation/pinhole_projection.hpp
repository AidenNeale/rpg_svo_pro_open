...
PinholeProjection<Distortion>::PinholeProjection(double fx, double fy, double cx, double cy,
...
template <typename Distortion>
PinholeProjection<Distortion>::PinholeProjection(const Eigen::VectorXd& intrinsics,
                                                 distortion_t distortion)
    : distortion_(distortion) {
  if (intrinsics.size() != 4) {
    throw std::runtime_error("PinholeProjection: expected 4 parameters, got " +
...
template <typename Distortion>
bool PinholeProjection<Distortion>::backProject3(const Eigen::Ref<const Eigen::Vector2d>& keypoint,
...
void PinholeProjection<Distortion>::project3(
  ...
  (*out_keypoint) = focal_matrix * distortion_.distort(uv) + Eigen::Vector2d(cx_, cy_);
...
template <typename Distortion>
PinholeProjection<Distortion> PinholeProjection<Distortion>::createTestProjection(
    const size_t image_width, const size_t image_height) {
  return PinholeProjection(image_width / 2, image_width / 2, image_width / 2, image_height / 2,
