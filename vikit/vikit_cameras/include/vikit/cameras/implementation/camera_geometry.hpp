...
CameraGeometry<Projection>::CameraGeometry(const int width, const int height,
...
template <typename Projection>
bool CameraGeometry<Projection>::backProject3(const Eigen::Ref<const Eigen::Vector2d>& keypoint,
                                              Eigen::Vector3d* out_point_3d) const {
  if (!out_point_3d) {
    throw std::runtime_error("CameraGeometry::backProject3: out_point_3d is null");
  }
...
template <typename Projection>
void CameraGeometry<Projection>::printParameters(std::ostream& out, const std::string& s) const {
  out << s << std::endl
...
template <typename Projection>
std::shared_ptr<CameraGeometry<Projection>> CameraGeometry<Projection>::createTestCamera() {
