...
class RadialTangentialDistortion {
 public:
  RadialTangentialDistortion(const double& k1, const double& k2, const double& p1, const double& p2)
      : k1_(k1), k2_(k2), p1_(p1), p2_(p2) {}

  RadialTangentialDistortion(const Eigen::VectorXd& parameters) {
    if (parameters.size() != 4) {
      throw std::runtime_error("RadialTangentialDistortion: expected 4 parameters, got " +
  ...
  inline Eigen::Vector2d distort(const Eigen::Vector2d& vector) const {
    ...
    const double r2 = xx + yy;
    const double cdist = (k1_ + k2_ * r2) * r2;
    return Eigen::Vector2d(vector(0) + vector(0) * cdist + p1_ * xy2 + p2_ * (r2 + 2.0 * xx),
  ...
  inline Eigen::Matrix2d jacobian(const Eigen::Vector2d& px_distorted) const {
    ...
    J_dist(0, 0) = cdist_p1 + k1_ * 2.0 * xx + k2_r2_x4 * xx + 2.0 * p1_ * px_distorted[1] +
                   6.0 * p2_ * px_distorted[0];
    J_dist(1, 1) = cdist_p1 + k1_ * 2.0 * yy + k2_r2_x4 * yy + 2.0 * p2_ * px_distorted[0] +
                   6.0 * p1_ * px_distorted[1];
    J_dist(1, 0) =
        2.0 * k1_ * xy + k2_r2_x4 * xy + 2.0 * p1_ * px_distorted[0] + 2.0 * p2_ * px_distorted[1];
  ...
  inline void print(std::ostream& out) const {
    out << "  Distortion: RadTan(" << k1_ << ", " << k2_ << ", " << p1_ << ", " << p2_ << ")"
        << std::endl;
  }
