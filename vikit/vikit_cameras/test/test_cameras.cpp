...
class CamerasTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ...
    if (data_dir_.empty()) {
      throw std::runtime_error("Failed to get data directory.");
    }
    if (!omni_cam_) {
      throw std::runtime_error("Failed to load omni cameras.");
    }
  }
...
TEST_F(CamerasTest, EquidistantDistortion) {
  double x = 0.5, y = 0.8;
  vk::cameras::EquidistantDistortion distortion_model(-0.0027, 0.0241, -0.0430, 0.0311);
  distortion_model.distort(x, y);
  ...
  struct EqJacobianFunctor : public aslam::common::NumDiffFunctor<2, 2> {
    EqJacobianFunctor(const vk::cameras::EquidistantDistortion& dist) : dist_(dist) {};
  ...
  TEST_JACOBIAN_FINITE_DIFFERENCE(EqJacobianFunctor, Eigen::Vector2d(x, y), kStepSize, kEpsJacobian,
                                  distortion_model);
}

TEST_F(CamerasTest, RadialTangentialDistortion) {
  double x = 0.5, y = 0.8;
  vk::cameras::RadialTangentialDistortion distortion_model(-0.3, 0.1, 9.52e-05, -0.00057);
  distortion_model.distort(x, y);
  distortion_model.undistort(x, y);
...
TEST_F(CamerasTest, CameraYaml) {
  ...
  if (!cam.get()) {
    throw std::runtime_error("Failed to load camera from yaml.");
  }
}
...
TEST(CameraTest, EquidistantProjection) {
  const double f = 350;
  vk::cameras::Camera::Ptr cam = std::make_shared<vk::cameras::EquidistantFisheyeGeometry>(
  ...
  EXPECT_THROW(cam->project3(Eigen::Vector3d(0.0, 0.0, 0.0), &projected), std::runtime_error);
...
TEST_F(CamerasTest, OmniCameraInitialization) {
  ...
  if (!omni_cam.get()) {
    throw std::runtime_error("OmniCameraInitialization: Failed to load omni camera from yaml.");
  }
...
TEST_F(CamerasTest, OmniCameraBackprojection) {
  ...
  const Eigen::Vector3d expected_bearing(0.733011271294813, 0.549758453471110, 0.400574735838165);
...
TEST_F(CamerasTest, OmniCameraProjection) {
  ...
  const Eigen::Vector2d expected_keypoint(4.729118411664447e+02, 3.929118411664447e+02);
...
TEST_F(CamerasTest, OmniCameraJacobian) {
  ...
  TEST_JACOBIAN_FINITE_DIFFERENCE(OmniJacobianFunctor, landmark, kStepSize, kEpsJacobian,
                                  omni_cam_);
}
