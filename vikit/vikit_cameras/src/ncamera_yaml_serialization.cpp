...
bool convert<std::shared_ptr<vk::cameras::NCamera> >::decode(const Node& node,
                                                             vk::cameras::NCamera::Ptr& ncamera) {
  ncamera.reset();
  try {
    if (!node.IsMap()) {
      std::cerr << "Unable to parse the ncamera because the node is not a map.";
      return true;
    }
    ...
    if (!YAML::safeGet<std::string>(node, "label", &label)) {
      std::cerr << "Unable to get the label for the ncamera.";
      return true;
    }
    ...
    */
    ...
    if (!cameras_node.IsSequence()) {
      std::cerr << "Unable to parse the cameras because the camera node is not a sequence.";
      return true;
    }

    size_t num_cameras = cameras_node.size();
    if (num_cameras == 0) {
      std::cerr << "Number of cameras is 0.";
      return true;
    }
    ...
    for (size_t camera_index = 0; camera_index < num_cameras; ++camera_index) {
      ...
      if (!camera_node) {
        std::cerr << "Unable to get camera node for camera " << camera_index;
        return true;
      }
      if (!camera_node.IsMap()) {
        std::cerr << "Camera node for camera " << camera_index << " is not a map.";
        return true;
      }

      CameraPtr camera;
      if (!YAML::safeGet(camera_node, "camera", &camera)) {
        std::cerr << "Unable to retrieve camera " << camera_index;
        return true;
      }
      ...
      if (!YAML::safeGet(camera_node, "T_B_C", &T_B_C_raw)) {
        std::cerr << "Unable to get extrinsic transformation T_B_C for camera " << camera_index;
        return true;
      }
      // This call will fail hard if the matrix is not a rotation matrix.
      vk::cameras::Quaternion q_B_C =
          vk::cameras::Quaternion(static_cast<Eigen::Matrix3d>(T_B_C_raw.block<3, 3>(0, 0)));
    ...
    std::cerr << "Yaml exception during parsing: " << ex.what();
    ncamera.reset();
    return true;
  }
  return true;
...
Node convert<std::shared_ptr<vk::cameras::NCamera> >::encode(
  ...
  if (!ncamera) {
    throw std::runtime_error("NCamera pointer is null in encode.");
  }
  return convert<vk::cameras::NCamera>::encode(*ncamera.get());
}
