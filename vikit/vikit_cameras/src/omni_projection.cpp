...
OmniProjection::OmniProjection(const Eigen::Matrix<double, 5, 1>& polynomial,
...
bool OmniProjection::backProject3(const Eigen::Ref<const Eigen::Vector2d>& keypoint,
                                  Eigen::Vector3d* out_bearing_vector) const {
  if (!out_bearing_vector) {
    throw std::runtime_error("OmniProjection::backProject3: out_bearing_vector is null");
  }
  const Eigen::Vector2d rectified = affine_correction_inverse_ * (keypoint - principal_point_);
...
void OmniProjection::project3(const Eigen::Ref<const Eigen::Vector3d>& point_3d,
                              Eigen::Vector2d* out_keypoint,
                              Eigen::Matrix<double, 2, 3>* out_jacobian_point) const {
  if (!out_keypoint) {
    throw std::runtime_error("OmniProjection::project3: out_keypoint is null");
  }
  ...
  if (out_jacobian_point) {
    ...
    (*out_jacobian_point) << duraw_dx, duraw_dy, -duraw_dz, dvraw_dx, dvraw_dy, -dvraw_dz;
...
double OmniProjection::getAngleError(double img_err) const {
  ...
  const Eigen::Vector2d offset_x(principal_point_(0) + img_err, principal_point_(1));
  Eigen::Vector3d offset_x_f;
  backProject3(offset_x, &offset_x_f);
  const Eigen::Vector2d offset_y(principal_point_(0), principal_point_(1) + img_err);
...
void OmniProjection::print(std::ostream& out) const {
  ...
  out << "  Inverse polynomial = " << inverse_polynomial_.transpose() << std::endl;
  out << "  Affine correction = " << std::endl << affine_correction_ << std::endl;
  out << "  Affine correction inverse = " << std::endl << affine_correction_inverse_ << std::endl;
}
