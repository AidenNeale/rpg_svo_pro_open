...
bool convert<std::shared_ptr<vk::cameras::CameraGeometryBase> >::decode(
  ...
  try {
    if (!node.IsMap()) {
      std::cerr << "Unable to get parse the camera because the node is not a map.";
      return true;
    }
      ...
      if (camera_type == "pinhole" && distortion_type == "none") {
        ...
        camera.reset(new vk::cameras::PinholeRadTanGeometry(
            ...
            vk::cameras::PinholeProjection<vk::cameras::RadialTangentialDistortion>(
                intrinsics, vk::cameras::RadialTangentialDistortion(distortion_parameters))));
        ...
        camera.reset(new vk::cameras::PinholeEquidistantGeometry(
            ...
            vk::cameras::PinholeProjection<vk::cameras::EquidistantDistortion>(
                intrinsics, vk::cameras::EquidistantDistortion(distortion_parameters))));
        ...
        camera.reset(new vk::cameras::PinholeAtanGeometry(
            ...
            vk::cameras::PinholeProjection<vk::cameras::AtanDistortion>(
                intrinsics, vk::cameras::AtanDistortion(distortion_parameters))));
      } else if (camera_type == "omni") {
        camera.reset(new vk::cameras::OmniGeometry(image_width, image_height, intrinsics));
      } else {
        std::cerr << "Unknown camera and distortion combination!";
      }
    }
    ...
    std::cerr << "Yaml exception during parsing: " << e.what();
    camera.reset();
    return true;
  }
  return true;
...
Node convert<vk::cameras::CameraGeometryBase::Ptr>::encode(
  ...
  if (!camera) {
    throw std::runtime_error("Camera pointer is null in encode.");
  }
  return convert<vk::cameras::CameraGeometryBase>::encode(*camera.get());
}

bool convert<vk::cameras::CameraGeometryBase>::decode(const Node& /*node*/,
...
template <>
void encodeDistortion(const vk::cameras::NoDistortion& distortion, Node* distortion_node) {
  if (!distortion_node) {
    std::runtime_error("encodeDistortion: distortion_node is null");
  }
...
template <>
void encodeDistortion(const vk::cameras::EquidistantDistortion& distortion, Node* distortion_node) {
  if (!distortion_node) {
    std::runtime_error("encodeDistortion: distortion_node is null");
  }
...
template <>
void encodeDistortion(const vk::cameras::AtanDistortion& distortion, Node* distortion_node) {
  if (!distortion_node) {
    std::runtime_error("encodeDistortion: distortion_node is null");
  }
...
template <typename DistortionType>
bool encodePinhole(const vk::cameras::CameraGeometryBase& camera, Node* camera_node) {
  ...
  if (!camera_node) {
    std::runtime_error("encodePinhole: camera_node is null");
  }
  if (ConstGeometryPtr pinhole_camera = dynamic_cast<ConstGeometryPtr>(&camera)) {
    ...
    const Projection* projection = pinhole_camera->template projection<Projection>();
    intrinsics << projection->fx_, projection->fy_, projection->cx_, projection->cy_;
...
Node convert<vk::cameras::CameraGeometryBase>::encode(
  ...
  if (!internal::encodePinhole<vk::cameras::NoDistortion>(camera, &camera_node) &&
      !internal::encodePinhole<vk::cameras::RadialTangentialDistortion>(camera, &camera_node) &&
      !internal::encodePinhole<vk::cameras::EquidistantDistortion>(camera, &camera_node) &&
      !internal::encodePinhole<vk::cameras::AtanDistortion>(camera, &camera_node)) {
    std::ostringstream oss;
    camera.printParameters(oss);
    std::cerr << "Camera encoding currently not supported, camera is " << std::endl << oss.str();
  }
