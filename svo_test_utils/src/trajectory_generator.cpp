  ...
  if (fs.is_open()) {
    while (fs >> stamp >> tx >> ty >> tz >> qx >> qy >> qz >> qw) {
      ...
      if (std::abs(q.norm() - 1.0) > 0.000001)
        std::cout << "trajectory::Trajectory: Quaternion norm = " << q.norm();
      q.normalize();
...
Trajectory Trajectory::createCircularTrajectory(size_t length, double fps,
...
Trajectory Trajectory::createStraightTrajectory(size_t length, double fps,
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    poses.push_back(
        StampedPose(stamp, Transformation(Eigen::Quaterniond(1.0, 0.0, 0.0, 0.0), current_pos)));
    stamp += 1.0 / fps;
  }
  return Trajectory(poses);
}
