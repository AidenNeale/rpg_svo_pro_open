...
SyntheticDataset::SyntheticDataset(const std::string& dataset_dir, size_t cam_index,
                                   size_t first_frame_id, double sigma_img_noise)
...
bool SyntheticDataset::getNextFrame(size_t n_pyramid_levels, FramePtr& frame, cv::Mat* depthmap) {
  ...
  if (img.empty()) {
    std::cerr << "Failed to read image: " << img_filename;
    return false;
  }
  ...
  if (sigma_img_noise_ > 0.0) {
    ...
    for (size_t i = 0; i < area; ++i, ++data_ptr) *data_ptr += distribution(mt);
  }
  ...
  if (depth_fs_.is_open()) {
    ...
    if (depth_fs_ >> depth_id >> depth_name) {
      ...
      if (depthmap) {
        ...
        vk::blender_utils::loadBlenderDepthmap(depth_filename.c_str(), cam_->imageWidth(),
...
void SyntheticDataset::init() {
  ...
  if (!img_fs_.is_open()) {
    throw std::runtime_error("Failed to load image list from '" + img_filename + "'");
  }
  ...
  if (!gt_fs_.is_open()) {
    std::cerr << "No groundtruth trajectory found.";
  }
  ...
  if (!depth_fs_.is_open()) {
    std::cerr << "No depthmaps trajectory found.";
  }
...
void SyntheticDataset::skipFrames(size_t first_frame_id) {
  while (img_fs_.good() && !img_fs_.eof()) {
    // skip comment block
    while (img_fs_.peek() == '#') img_fs_.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  ...
  std::cerr << "Image list corrupted or first_frame_id exceeds rang listed.";
}
