...
void detectEdges(const std::vector<cv::Mat>& img_pyr, const double sigma, cv::Mat& edge_map,
                 cv::Mat& level_map) {
  //  printf("detect edges\n");
...
void getCovarEntries(const cv::Mat& src, cv::Mat& dxdx, cv::Mat& dydy, cv::Mat& dxdy) {
#ifdef __SSSE3__
  ...
  __m128i mask_hi = _mm_set_epi8(0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00,
                                 0xFF, 0x00, 0xFF, 0x00, 0xFF);
  __m128i mask_lo = _mm_set_epi8(0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF,
  ...
  for (unsigned int i = 0; i < maxI; ++i) {
    ...
    for (unsigned int j = 0; j < maxJ;) {
      ...
      // calculate covariance entries - remove precision (ends up being 4 bit), then remove 4 more
      // bits
