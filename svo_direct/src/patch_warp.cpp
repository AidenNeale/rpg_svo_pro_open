...
void getWarpMatrixAffine(const CameraPtr& cam_ref, const CameraPtr& cam_cur,
  ...
  cam_ref->backProject3(px_ref + Eigen::Vector2d(kHalfPatchSize, 0) * (1 << level_ref),
                        &xyz_du_ref);
  cam_ref->backProject3(px_ref + Eigen::Vector2d(0, kHalfPatchSize) * (1 << level_ref),
                        &xyz_dv_ref);
...
void getWarpMatrixAffineHomography(const CameraPtr& cam_ref, const CameraPtr& cam_cur,
                                   const Keypoint& px_ref, const BearingVector& f_ref,
                                   const BearingVector& normal_ref, const double depth_ref,
                                   const Transformation T_cur_ref, const int level_ref,
...
int getBestSearchLevel(const AffineTransformation2& A_cur_ref, const int max_level) {
...
bool warpAffine(const AffineTransformation2& A_cur_ref, const cv::Mat& img_ref,
                const Eigen::Ref<Keypoint>& px_ref, const int level_ref, const int search_level,
  ...
  for (int y = -halfpatch_size; y < halfpatch_size; ++y) {
    for (int x = -halfpatch_size; x < halfpatch_size; ++x, ++patch_ptr) {
      ...
      if (xi < 0 || yi < 0 || xi + 1 >= img_ref.cols || yi + 1 >= img_ref.rows)
        ...
        *patch_ptr = static_cast<uint8_t>(w00 * ptr[0] + w01 * ptr[stride] + w10 * ptr[1] +
                                          w11 * ptr[stride + 1]);
      }
    }
  }
...
bool warpPixelwise(const Frame& cur_frame, const Frame& ref_frame, const FeatureWrapper& ref_ftr,
                   const int level_ref, const int level_cur, const int halfpatch_size,
                   uint8_t* patch) {
