...
Matcher::MatchResult Matcher::findMatchDirect(const Frame& ref_frame, const Frame& cur_frame,
                                              const FeatureWrapper& ref_ftr,
                                              const FloatType& ref_depth, Keypoint& px_cur) {
  ...
  warp::getWarpMatrixAffine(ref_frame.cam_, cur_frame.cam_, ref_ftr.px, ref_ftr.f, ref_depth,
                            cur_frame.T_cam_world() * ref_frame.T_world_cam(), ref_ftr.level,
                            &A_cur_ref_);
  ...
  */
  if (options_.use_affine_warp_) {
    if (!warp::warpAffine(A_cur_ref_, ref_frame.img_pyr_[ref_ftr.level], ref_ftr.px, ref_ftr.level,
                          search_level_, kHalfPatchSize + 1, patch_with_border_))
      return MatchResult::kFailWarp;
  } else {
    // pixelwise warp:
    // TODO(zzc): currently using the search level from affine, good enough?
    if (!warp::warpPixelwise(cur_frame, ref_frame, ref_ftr, ref_ftr.level, search_level_,
  ...
  if (isEdgelet(ref_ftr.type)) {
    ...
    dir_cur.normalize();
    if (feature_alignment::align1D(cur_frame.img_pyr_[search_level_], dir_cur, patch_with_border_,
    ...
    bool res = feature_alignment::align2D(
        cur_frame.img_pyr_[search_level_], patch_with_border_, patch_, options_.align_max_iter,
        options_.affine_est_offset_, options_.affine_est_gain_, px_scaled, false, last_fail_steps);
    if (res) {
      if ((px_scaled - px_scaled_start).norm() > options_.max_patch_diff_ratio * kPatchSize) {
        return MatchResult::kFailTooFar;
      }
...
Matcher::MatchResult Matcher::findEpipolarMatchDirect(
    const Frame& ref_frame, const Frame& cur_frame, const FeatureWrapper& ref_ftr,
    const double d_estimate_inv, const double d_min_inv, const double d_max_inv, double& depth) {
  Transformation T_cur_ref = cur_frame.T_f_w_ * ref_frame.T_f_w_.inverse();
  return findEpipolarMatchDirect(ref_frame, cur_frame, T_cur_ref, ref_ftr, d_estimate_inv,
                                 d_min_inv, d_max_inv, depth);
}

Matcher::MatchResult Matcher::findEpipolarMatchDirect(
    const Frame& ref_frame, const Frame& cur_frame, const Transformation& T_cur_ref,
    const FeatureWrapper& ref_ftr, const double d_estimate_inv, const double d_min_inv,
  ...
  const BearingVector A =
      T_cur_ref.getRotation().rotate(ref_ftr.f) + T_cur_ref.getPosition() * d_min_inv;
  const BearingVector B =
      T_cur_ref.getRotation().rotate(ref_ftr.f) + T_cur_ref.getPosition() * d_max_inv;
  ...
  warp::getWarpMatrixAffine(ref_frame.cam_, cur_frame.cam_, ref_ftr.px, ref_ftr.f,
                            1.0 / std::max(0.000001, d_estimate_inv), T_cur_ref, ref_ftr.level,
                            &A_cur_ref_);
  ...
  BearingVector C =
      T_cur_ref.getRotation().rotate(ref_ftr.f) + T_cur_ref.getPosition() * d_estimate_inv;
...
Matcher::MatchResult Matcher::findLocalMatch(const Frame& frame,
                                             const Eigen::Ref<GradientVector>& direction,
                                             const int patch_level, Keypoint& px_cur) {
  Keypoint px_scaled(px_cur / (1 << patch_level));
  bool res;
  if (options_.align_1d)
    res = feature_alignment::align1D(frame.img_pyr_[patch_level], direction, patch_with_border_,
                                     patch_, options_.align_max_iter, options_.affine_est_offset_,
                                     options_.affine_est_gain_, &px_scaled, &h_inv_);
  else
    res = feature_alignment::align2D(frame.img_pyr_[patch_level], patch_with_border_, patch_,
...
bool Matcher::updateZMSSD(const Frame& frame, const Eigen::Vector2i& pxi, const int patch_level,
...
bool Matcher::isPatchWithinImage(const Frame& frame, const Eigen::Vector2i& pxi,
...
void Matcher::scanEpipolarLine(const Frame& frame, const Eigen::Vector3d& A,
...
void Matcher::scanEpipolarUnitPlane(const Frame& frame, const Eigen::Vector3d& A,
                                    const Eigen::Vector3d& B, const Eigen::Vector3d& C,
                                    const PatchScore& patch_score, const int patch_level,
                                    Keypoint* image_best, int* zmssd_best) {
  ...
  for (size_t i = 0; i < n_steps; ++i, uv += step) {
    ...
    if (updateZMSSD(frame, pxi, patch_level, patch_score, zmssd_best)) uv_best = uv;
...
void Matcher::scanEpipolarUnitSphere(const Frame& frame, const Eigen::Vector3d& A,
                                     const Eigen::Vector3d& B, const Eigen::Vector3d& C,
                                     const PatchScore& patch_score, const int patch_level,
                                     Keypoint* image_best, int* zmssd_best) {
  ...
  for (size_t i = 0; i < n_steps; i++) {
    ...
    // update ZMSSD
    if (updateZMSSD(frame, pxi, patch_level, patch_score, zmssd_best)) f_best = f;
  }
...
Matcher::MatchResult depthFromTriangulation(const Transformation& T_search_ref,
  ...
  if (AtA.determinant() < 0.000001) return Matcher::MatchResult::kFailTriangulation;
