...
DepthFilter::DepthFilter(const DepthFilterOptions& options, const DetectorOptions& detector_options,
  ...
  if (options_.extra_map_points) {
    ...
    sec_feature_detector_ =
        feature_detection_utils::makeDetector(sec_detector_options, cams->getCameraShared(0));
  }
}
...
void DepthFilter::addKeyframe(const FramePtr& frame, const double depth_mean,
  ...
  if (thread_ == nullptr) {
    ...
    depth_filter_utils::initializeSeeds(frame, feature_detector_, options_.max_n_seeds_per_frame,
    ...
    if (options_.extra_map_points) {
      ...
      depth_filter_utils::initializeSeeds(
          ...
          options_.max_n_seeds_per_frame + options_.max_map_seeds_per_frame, depth_min, depth_max,
          depth_mean);
    }
  } else {
...
void DepthFilter::updateSeedsLoop() {
  while (true) {
    ...
    if (job.type == Job::SEED_INIT) {
      ulock_t lock(feature_detector_mut_);
      depth_filter_utils::initializeSeeds(job.cur_frame, feature_detector_,
                                          options_.max_n_seeds_per_frame, job.min_depth,
                                          job.max_depth, job.mean_depth);
      if (options_.extra_map_points) {
        ...
        depth_filter_utils::initializeSeeds(
            ...
            options_.max_n_seeds_per_frame + options_.max_map_seeds_per_frame, job.min_depth,
            job.max_depth, job.mean_depth);
      }
      ...
      depth_filter_utils::updateSeed(*job.cur_frame, *job.ref_frame, job.ref_frame_seed_index,
...
size_t DepthFilter::updateSeeds(const std::vector<FramePtr>& ref_frames_with_seeds,
  ...
  if (thread_ == nullptr) {
    for (const FramePtr& ref_frame : ref_frames_with_seeds) {
      for (size_t i = 0; i < ref_frame->num_features_; ++i) {
        ...
        if (isSeed(type)) {
          ...
          if (type == FeatureType::kMapPointSeed || type == FeatureType::kMapPointSeedConverged) {
            cur_thresh = options_.mappoint_convergence_sigma2_thresh;
          }
          // We get higher precision (10x in the synthetic blender dataset)
          // when we keep updating seeds even though they are converged until
          // the frame handler selects a new keyframe.
          if (depth_filter_utils::updateSeed(*cur_frame, *ref_frame, i, *matcher_, cur_thresh, true,
                                             false)) {
            ++n_success;
          }
        }
    ...
    SVO_DEBUG_STREAM("DepthFilter: " << cur_frame->cam()->getLabel() << " updated " << n_success
                                     << " Seeds successfully.");
  } else {
    ulock_t lock(jobs_mut_);
    for (const FramePtr& ref_frame : ref_frames_with_seeds) {
...
void initializeSeeds(const FramePtr& frame, const AbstractDetector::Ptr& feature_detector,
                     const size_t max_n_seeds, const float depth_min, const float depth_max,
  ...
  SVO_DEBUG_STREAM("DepthFilter: " << frame->cam()->getLabel() << " Initialized " << n_new
                                   << " new seeds");
}

bool updateSeed(const Frame& cur_frame, Frame& ref_frame, const size_t& seed_index,
                Matcher& matcher, const FloatType sigma2_convergence_threshold,
                const bool check_visibility, const bool check_convergence,
  ...
  if ((type == FeatureType::kCornerSeedConverged || type == FeatureType::kEdgeletSeedConverged ||
  ...
  if (check_visibility) {
    ...
    Eigen::Vector2d px;
    if (!cur_frame.cam()->project3(xyz_f, &px).isKeypointVisible()) return false;

    // check margin
    const Eigen::Vector2i pxi = px.cast<int>();
    const int boundary = 9;

    if (!cur_frame.cam()->isKeypointVisibleWithMargin(pxi, boundary)) return false;
  }
  ...
  if (use_vogiatzis_update) {
    if (!updateFilterVogiatzis(seed::getMeanFromDepth(depth),
    ...
    if (!updateFilterGaussian(seed::getMeanFromDepth(depth),
...
bool updateFilterVogiatzis(const FloatType z,  // Measurement
                           const FloatType tau2, const FloatType mu_range,
...
bool updateFilterGaussian(const FloatType z,  // Measurement
                          const FloatType tau2, Eigen::Ref<SeedState>& mu_sigma2_a_b) {
  FloatType& mu = mu_sigma2_a_b(0);
  FloatType& sigma2 = mu_sigma2_a_b(1);
  FloatType& a = mu_sigma2_a_b(2);
  FloatType& b = mu_sigma2_a_b(3);
...
double computeTau(const Transformation& T_ref_cur, const BearingVector& f, const FloatType z,
...
double computeEpiGradAngle(const Transformation& T_cur_ref, const BearingVector& f_ref,
                           const GradientVector& grad_ref, const FloatType depth_estimate) {
  ...
  const BearingVector f_ref_plus(f_ref +
                                 BearingVector(0.1 * grad_ref[0], 0.1 * grad_ref[1], f_ref[2]));
...
bool setSeedCovariance(const int halfpatch_size, const double image_noise2,
  ...
  if (u - halfpatch_size < 0 || u + halfpatch_size >= img.cols || v - halfpatch_size < 0 ||
      v + halfpatch_size >= img.rows)
    return false;
  ...
  H /= 260100.0 * patch_size * patch_size;  // 260100 = 2^2 * 255^2 for the missing 0.5 of the
                                            // derivative and the 255 of uint8_t image
...
double getSeedDisparityUncertainty(const SeedImplementation::Ptr& seed,
  ...
  double disp_sigma2 = seed->patch_cov_.determinant() /
                       (seed->patch_cov_(1, 1) * c * c + 2 * seed->patch_cov_(0, 1) * s * c +
                        seed->patch_cov_(0, 0) * s * s);
  return fmax(disp_sigma2,
              1.0);  // we don't want less than 1.0px uncertainty. because e.g. sampling errors
}
#endif
