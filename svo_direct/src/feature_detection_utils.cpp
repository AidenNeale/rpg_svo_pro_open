...
//------------------------------------------------------------------------------
AbstractDetector::Ptr makeDetector(const DetectorOptions& options, const CameraPtr& cam) {
...
//------------------------------------------------------------------------------
void fillFeatures(const Corners& corners, const FeatureType& type, const cv::Mat& mask,
  ...
  for (const Corner& c : corners) {
    if (c.score > threshold) {
      ...
      // feature extraction.
      if (!mask.empty() && mask.at<uint8_t>(static_cast<int>(c.y), static_cast<int>(c.x)) == 0) {
        continue;
      }
  ...
  std::sort(idx.begin(), idx.end(),
            [&score_vec](size_t i1, size_t i2) { return score_vec[i1] > score_vec[i2]; });
...
void fastDetector(const ImgPyr& img_pyr, const int threshold, const int border,
  ...
  for (size_t level = min_level; level <= max_level; ++level) {
...
#if __SSE2__
    fast::fast_corner_detect_10_sse2((fast::fast_byte*)img_pyr[level].data, img_pyr[level].cols,
                                     img_pyr[level].rows, img_pyr[level].step, threshold,
                                     fast_corners);
#elif HAVE_FAST_NEON
    fast::fast_corner_detect_9_neon((fast::fast_byte*)img_pyr[level].data, img_pyr[level].cols,
                                    img_pyr[level].rows, img_pyr[level].step, threshold,
                                    fast_corners);
#else
    fast::fast_corner_detect_10((fast::fast_byte*)img_pyr[L].data, img_pyr[L].cols, img_pyr[L].rows,
    ...
    for (const int& i : nm_corners) {
      ...
      if (xy.x < border || xy.y < border || xy.x >= maxw || xy.y >= maxh) continue;
...
void shiTomasiDetector(const ImgPyr& img_pyr, const int threshold, const int border,
  ...
  for (size_t level = min_level; level <= max_level; ++level) {
    ...
    for (const int& i : nm_corners) {
      ...
      const size_t close_k = closeness_check_grid.getCellIndex(xy.x(), xy.y(), scale);
...
bool getShiTomasiScore(const cv::Mat& img, const Eigen::Vector2i& px, double* score) {
  ...
  if (x_min < 1 || x_max >= img.cols - 1 || y_min < 1 || y_max >= img.rows - 1) return false;
...
//------------------------------------------------------------------------------
void computeDerivHuangMumford(const cv::Mat& img_8u, cv::Mat& mag_32f, float alpha, float q) {
  ...
  for (int y = 0; y < sz.height; ++y) {
    ...
    for (int x = 0; x < sz.width; ++x) {
      ...
      img_xp /= 255.0f;
      img_yp /= 255.0f;
      float dx = std::max(1e-3f, std::exp(-alpha * std::pow(std::abs(img_xp - img_c), q)));
      float dy = std::max(1e-3f, std::exp(-alpha * std::pow(std::abs(img_yp - img_c), q)));
      //      rgx[x] = dx;
      //      rgy[x] = dy;
...
//------------------------------------------------------------------------------
void displayGrid(const OccupandyGrid2D& old_grid, const Keypoints& keypoints, const int img_width,
  ...
  for (int i = 0; i < keypoints.cols(); ++i) {
    new_grid.occupancy_.at(new_grid.getCellIndex(keypoints(0, i), keypoints(1, i))) = true;
  }
...
void nonlinearDiffusion(const cv::Mat& img_8u, cv::Mat& img_8u_diffused, const double timestep,
...
//------------------------------------------------------------------------------
void detectCannyEdges(const cv::Mat& src_gray, cv::Mat& dest, int low_threshold, int ratio,
...
//------------------------------------------------------------------------------
void detectSobelEdges(const cv::Mat& src_gray, cv::Mat& dest, int low_threshold, int kernel_size) {
  int scale = 1;
  int delta = 0;
  int ddepth = CV_16S;
  ...
  *img_rgb = cv::Mat(frame.img_pyr_[level].size(), CV_8UC3);
  ...
  if (level == 0) {
    for (size_t i = 0; i < frame.num_features_; ++i) {
      ...
      if (frame.landmark_vec_[i] == nullptr && frame.seed_ref_vec_[i].keyframe == nullptr &&
      ...
      switch (frame.type_vec_[i]) {
        case FeatureType::kEdgelet:
          cv::line(*img_rgb, cv::Point2f(px(0) + 3 * g(1), px(1) - 3 * g(0)),
                   cv::Point2f(px(0) - 3 * g(1), px(1) + 3 * g(0)), cv::Scalar(255, 0, 255), 2);
          break;
        case FeatureType::kCorner:
          cv::rectangle(*img_rgb, cv::Point2f(px(0) - 2, px(1) - 2),
                        cv::Point2f(px(0) + 2, px(1) + 2), cv::Scalar(0, 255, 0), -1);
          break;
        case FeatureType::kMapPoint:
          cv::rectangle(*img_rgb, cv::Point2f(px(0) - 2, px(1) - 2),
                        cv::Point2f(px(0) + 2, px(1) + 2), cv::Scalar(255, 0, 0), -1);
          break;
        case FeatureType::kFixedLandmark:
          cv::rectangle(*img_rgb, cv::Point2f(px(0) - 3, px(1) - 3),
                        cv::Point2f(px(0) + 3, px(1) + 3), cv::Scalar(101, 236, 255), -1);
          break;
        ...
        case FeatureType::kMapPointSeedConverged:
          cv::circle(*img_rgb, cv::Point2f(px(0), px(1)), 5, cv::Scalar(255, 0, 0), 1);
          break;
        default:
    ...
    for (size_t i = 0; i < frame.num_features_; ++i) {
      if (frame.isValidLandmark(i)) {
        const auto& px = frame.px_vec_.col(i);
        cv::rectangle(*img_rgb, cv::Point2f(px(0) / scale - size, px(1) / scale - size),
...
double getAngleAtPixelUsingHistogram(const cv::Mat& img, const Eigen::Vector2i& px,
...
void nonmax_3x3(const std::vector<Eigen::Vector2i>& corners, const std::vector<int>& scores,
  ...
  for (int i = 0; i < sz; i++) {
    ...
    if (i > 0)
      ...
      if (corners[i - 1].y() == pos.y() - 1 && corners[i - 1].x() == pos.x() &&
          scores[i - 1] >= score)
        continue;
    ...
    if (pos.x() != 0 && row_start[pos.x() - 1] != -1) {
      ...
      // row above.
      if (corners[point_above].x() < pos.x() - 1) point_above = row_start[pos.x() - 1];

      // Make point_above point to the first of the pixels above the current point,
      // if it exists.
      for (; corners[point_above].x() < pos.x() && corners[point_above].y() < pos.y() - 1;
           point_above++) {
      }
    ...
    {
      if (corners[point_below].x() < pos.x() + 1) point_below = row_start[pos.x() + 1];

      // Make point below point to one of the pixels belowthe current point, if it
      // exists.
      for (; point_below < sz && corners[point_below].x() == pos.x() + 1 &&
             corners[point_below].y() < pos.y() - 1;
           point_below++) {
      }

      for (int i = point_below;
           i < sz && corners[i].x() == pos.x() + 1 && corners[i].y() <= pos.y() + 1; i++) {
        int y = corners[i].y();
        if ((y == pos.y() - 1 || y == pos.y() || y == pos.y() + 1) && (scores[i] >= score))
          goto cont;
      }
...
void angleHistogram(const cv::Mat& img, int x, int y, int halfpatch_size, AngleHistogram& hist) {
...
bool gradientAndMagnitudeAtPixel(const cv::Mat& img, int x, int y, double* mag, double* angle) {
