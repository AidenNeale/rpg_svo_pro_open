...
bool align1D(
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    int u_r = std::floor(u);
    int v_r = std::floor(v);
    if (u_r < kHalfPatchSize || v_r < kHalfPatchSize || u_r >= cur_img.cols - kHalfPatchSize ||
        v_r >= cur_img.rows - kHalfPatchSize)
      break;

    if (std::isnan(u) || std::isnan(v))  // TODO very rarely this can happen, maybe H is singular?
    ...
    for (int y = 0; y < kPatchSize; ++y) {
      uint8_t* it =
          (uint8_t*)cur_img.data + (v_r + y - kHalfPatchSize) * cur_step + u_r - kHalfPatchSize;
    ...
    patch_utils::concatenatePatches(
        {ref_patch_normalized, cur_patch_normalized, res_patch_normalized}, &concatenated);
    cv::line(concatenated, cv::Point2f(concatenated.rows / 2, concatenated.rows / 2),
...
//------------------------------------------------------------------------------
bool align2D_SSE2(const cv::Mat& cur_img, uint8_t* ref_patch_with_border, uint8_t* ref_patch,
                  const int n_iter, Keypoint& cur_px_estimate) {
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    __m128i qw0 = _mm_set1_epi32(
        wTL + (wTR << 16));  // Sets the 4 signed 32-bit integer values to [wTL, wTR].
    ...
    for (int y = 0; y < patch_size; ++y) {
      const uint8_t* it = (const uint8_t*)cur_img.data + (v_r + y - halfpatch_size) * cur_step +
                          u_r - halfpatch_size;
      ...
      // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned 16-bit integers in a with
      // the lower 4 signed or unsigned 16-bit integers in b. _mm_madd_epi16: Multiplies the 8
      // signed 16-bit integers from a by the 8 signed 16-bit integers from b.
      __m128i t0 = _mm_add_epi32(_mm_madd_epi16(_mm_unpacklo_epi16(v00, v01), qw0),
                                 _mm_madd_epi16(_mm_unpacklo_epi16(v10, v11), qw1));
      __m128i t1 = _mm_add_epi32(_mm_madd_epi16(_mm_unpackhi_epi16(v00, v01), qw0),
                                 _mm_madd_epi16(_mm_unpackhi_epi16(v10, v11), qw1));

      // _mm_srai_epi32: Shifts the 4 signed 32-bit integers in A right by count bits while shifting
      // in the sign bit.
      ...
      // compute 8xres:
      // _mm_packs_epi32: Packs the 8 signed 32-bit integers from a and b into signed 16-bit
      // integers and saturates.
      diff = _mm_subs_epi16(_mm_packs_epi32(t0, t1), diff);

      // load gradient dX and dY, both are aligned!
      v00 = _mm_load_si128(
          (const __m128i*)(ref_patch_dx + y * patch_size));  // [dx1, dx2, dx3, dx4 ...]
      v01 = _mm_load_si128(
          (const __m128i*)(ref_patch_dy + y * patch_size));  // [dy1, dy2, dy3, dy4 ...]

      // _mm_mulhi_epi16: Multiplies the 8 signed 16-bit integers from a by the 8 signed 16-bit
      // integers from b.
      v10 = _mm_mullo_epi16(
          v00, diff);  // Packs the lower 16 bits of the 8 signed 32-bit results. [15:0]
      v11 = _mm_mulhi_epi16(
          v00, diff);  // Packs the upper 16 bits of the 8 signed 32-bit results. [31:16]

      // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned 16-bit integers with the
...
#if SUBPIX_VERBOSE
    cout << "Iter " << iter << ":"
         << "\t u=" << u << ", v=" << v << "\t update = " << update_u << ", " << update_v
         << "\t new chi2 = " << new_chi2 << endl;
#endif
...
//------------------------------------------------------------------------------
bool align2D_NEON(const cv::Mat& cur_img, uint8_t* ref_patch_with_border, uint8_t* ref_patch,
                  const int n_iter, Keypoint& cur_px_estimate) {
  const int halfpatch_size = 4;
  const int patch_size = 8;
  const int patch_area = 64;
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    const uint16_t wTL =
        static_cast<uint16_t>((1.0f - subpix_x) * (1.0f - subpix_y) * (1 << SHIFT_BITS));
...
#if SUBPIX_VERBOSE
    cout << "Iter " << iter << ":"
         << "\t u=" << u << ", v=" << v << "\t update = " << update[0] << ", " << update[1] << endl;
#endif
...
//------------------------------------------------------------------------------
void alignPyr2DVec(const std::vector<cv::Mat>& img_pyr_ref, const std::vector<cv::Mat>& img_pyr_cur,
                   const int max_level, const int min_level, const std::vector<int>& patch_sizes,
                   const int n_iter, const float min_update_squared,
                   const std::vector<cv::Point2f>& px_ref, std::vector<cv::Point2f>& px_cur,
                   std::vector<uint8_t>& status) {
  for (size_t i = 0; i < px_ref.size(); ++i) {
    Eigen::Vector2i px_ref_level_0(px_ref[i].x, px_ref[i].y);
    Keypoint px_cur_level_0(px_cur[i].x, px_cur[i].y);
    bool res = alignPyr2D(img_pyr_ref, img_pyr_cur, max_level, min_level, patch_sizes, n_iter,
                          min_update_squared, px_ref_level_0, px_cur_level_0);
    status[i] = res ? 1 : 0;
...
//------------------------------------------------------------------------------
bool alignPyr2D(const std::vector<cv::Mat>& img_pyr_ref, const std::vector<cv::Mat>& img_pyr_cur,
                const int max_level, const int min_level, const std::vector<int>& patch_sizes,
                const int n_iter, const float min_update_squared,
                const Eigen::Vector2i& px_ref_level_0, Keypoint& px_cur_level_0) {
...
#define SVO_DESCALE(x, n) (((x) + (1 << ((n) - 1))) >> (n))
  ...
  for (int level = max_level; level >= min_level; --level) {
    ...
    const Eigen::Vector2f px_ref_flt =
        px_ref_level_0.cast<float>() / scale - Eigen::Vector2f(halfpatch_size, halfpatch_size);
    const Eigen::Vector2i px_ref = px_ref_flt.cast<int>();
    const Eigen::Vector2f px_ref_offset = px_ref_flt - px_ref.cast<float>();

    if (px_ref[0] < 1 || px_ref[1] < 1 || px_ref[0] >= width - patch_size - 1 ||
    ...
    for (int iter = 0; iter < n_iter; ++iter) {
      ...
      for (int y = 0; y < patch_size; ++y) {
        ...
        for (int x = 0; x < patch_size; ++x, ++it, ++it_ref, ++it_ref_dx, ++it_ref_dy) {
          uint16_t cur = SVO_DESCALE(
              wTL * it[0] + wTR * it[1] + wBL * it[step] + wBR * it[step + 1], SHIFT_BITS);
      ...
      update = Hinv * Jres *
               2.0;  // * 2 to compensate because above, we did not compute the derivative correctly
      u += update[0];
      v += update[1];
