...
cv::Mat CannyThreshold(cv::Mat& src_gray, int lowThreshold = 20, int ratio = 3,
                       int kernel_size = 3) {
...
int main(int argc, char** argv) {
  ...
  while (dataset.getNextFrame(5u, frame, nullptr)) {
    ...
    svo::CameraPtr cam =
        aslam::createCamera<aslam::PinholeCamera>(intrinsics, img_src.cols, img_src.rows);
    ...
    cv::threshold(mag_level_32f, mag_level_thresholded, detectorOptions.threshold_primary, 1.0,
