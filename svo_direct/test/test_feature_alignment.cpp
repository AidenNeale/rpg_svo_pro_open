...
void testAlignment1D_sameImg() {
  ...
  for (int i = 0; i < ref_frame->px_vec_.cols(); ++i) {
    ...
    warp::createPatchNoWarpInterpolated(ref_frame->img_pyr_[0], kp_ref, kHalfPatchSize + 1,
    ...
    bool res =
        feature_alignment::align1D(cur_frame->img_pyr_[0], ref_frame->grad_vec_.col(i),
...
void testAlignment1D_differentImg() {
  ...
  for (int i = 0; i < ref_frame->px_vec_.cols(); ++i) {
    ...
    if (!cur_frame->cam()->isKeypointVisibleWithMargin(px_cur, 10)) continue;
