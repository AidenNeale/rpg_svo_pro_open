...
class Dataset {
 public:
  Dataset(const std::string& dataset_dir, size_t cam_index = 0, size_t first_frame_id = 0)
      : dataset_dir_(dataset_dir), first_frame_id_(first_frame_id), cam_index_(cam_index) {
    init();
  }
  ...
  svo::FramePtr getNextImage(size_t img_align_max_level, int user_id) {
    ...
    // make frame
    svo::FramePtr frame =
        std::make_shared<svo::Frame>(cam_, img.clone(), stamp, img_align_max_level + 1, user_id);
...
#if 1
  return std::string(ament_index_cpp::get_package_share_directory("svo") + "/test/data");
#else
  const char* env_dir = std::getenv("SVO_DATASET_DIR");
...
inline cv::Mat drawHist(const std::vector<size_t>& hist, const std::vector<double>& bounds,
  ...
  for (size_t i = 0; i < hist_bin_num; i++) {
    ...
    cv::rectangle(histogram, cv::Point2f(bin_bl + 0.1 * bin_width, height),
                  cv::Point2f(bin_bl + 0.9 * bin_width, height - bin_height), cv::Scalar(0),
                  cv::FILLED);
    cv::putText(histogram, ss.str(), cv::Point2f(bin_bl + 0.1 * bin_width, height / 2.0),
                cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(120), 2);
    cv::putText(histogram, std::to_string(hist[i]),
                cv::Point2f(bin_bl + 0.1 * bin_width, height * 0.8 / 2.0), cv::FONT_HERSHEY_PLAIN,
                1.2, cv::Scalar(120), 2);
  }
  return histogram;
}
