...
struct DepthJacobianFunctor : public aslam::common::NumDiffFunctor<2, 1> {
  DepthJacobianFunctor(const svo::Camera::ConstPtr& camera, const Eigen::Vector3d& f_ref,
  ...
  virtual bool functional(const typename aslam::common::NumDiffFunctor<1, 1>::InputType& inv_depth,
    ...
    if (Jout) {
      ...
      *Jout = projection_jacobian *
              T_cur_ref_.getRotation().rotate(f_ref_ * (-1.0 / std::pow(inv_depth(0, 0), 2)));
    }
...
TEST(TestDepthEstimation, JacobianWrtInverseDepth) {
  ...
  TEST_JACOBIAN_FINITE_DIFFERENCE(DepthJacobianFunctor, inv_depth, 1e-8, 1e-6, camera, f_ref,
                                  T_cur_ref);
}
