...
inline void createPatchFromPatchWithBorder(const uint8_t* const patch_with_border,
                                           const int patch_size, uint8_t* patch) {
  ...
  for (int y = 1; y < patch_size + 1; ++y, patch_ptr += patch_size) {
    ...
    for (int x = 0; x < patch_size; ++x) patch_ptr[x] = ref_patch_border_ptr[x];
  }
}
...
inline void concatenatePatches(std::vector<cv::Mat> patches, cv::Mat* result_rgb) {
